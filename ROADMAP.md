...
| 9.6 | `[~]` | **Half closed. The mob half is done by 10.14** — the creatures on the shelves were not missing, they were being drawn in the valley 78 studs below their own health plates; every raised creature now stands on its shelf (201/201). **The climb-legibility half is NOT verified** — `TerraceRamp` and the per-tier stairs exist and are walkable, but nobody has checked that the route up reads as a route without hunting. That is what is left of this row. **Zones: real mobs everywhere, and legible climb routes.** The terraces already carry `TerraceRamp` + stairs per tier per side; the job is making "higher = harder = better" readable, and finding why a streamed-in creature can read as a floating health plate | walk a zone: every plate has a body under it; the route up is obvious without hunting |
