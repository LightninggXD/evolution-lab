...
- **`tools/codediff.py` — use it on any diff whose summary is "mostly comments".** It strips comments
  and blank lines from two revisions and diffs what is left, and it exists because **19.6 found a
  compaction that deleted two live guards while looking like a comment-only pass.** 391 changed lines
  is far too many to eyeball; stripped, the same diff was five lines and the guards were obvious.
  `--quarantine` sweeps `tools/_studio_pull/` against `src/` (measured 2026-08-20: eleven of twelve
  code-identical, only `FirstJoin` diverged, which is what made that incident isolated rather than
  systemic). It is not a linter and makes no judgement -- a rename or a reflow shows up too, so a
  non-empty result is a prompt to read, not a verdict.
...
**⚠️ THE TWO PARAGRAPHS THAT STOOD HERE SAID NOTHING COULD REACH TRADING, AND THAT HAS NOT BEEN
TRUE FOR SOME TIME.** Corrected 2026-08-19 (row 19.10). `ServerMain` requires `TradeService` at
line 34 and calls `Init()` at line 135; `Init` creates all seven remotes — `TradeUpdate`,
`TradeInvite`, `TradeRequest`, `TradeAccept`, `TradeCancel`, `TradeSetOffer`, `TradeConfirm` — and
connects the five inbound ones. The client half is `HUD/TradePanel` (1,011 lines), required by
MainUI. **Row 8.6 is `[x]` in the table below**, which is the contradiction that should have caught
this: a preamble and its own table disagreed, and the preamble is what a cold agent reads first.
The gate was opened when the place was published; only the comment was left behind.

**What IS still true, and is a different problem: there is no HUD tile for trading.** The only door
is knowing to click another player in the world. Shipped-and-undiscoverable is not the same as
unwired, and row **21.1** is where that is fixed.
...
| 18.6 | `[x]` | <!-- Kristina 2026-08-16, with a screenshot of the Season panel: "ovde imaju 2 progres bara sta ce mi, popravi to do kraja i nabudzi u nekim bojama, sve panele odradi pa mi onda pisi, sve hocu da bude top" --> **The same three faults, measured across all nineteen panels at once.** Her screenshot is the Season panel showing the *same number twice* — the level card's `SeasonXP` bar and the 30-segment `Rail` between the free and premium rows are both progress readouts of the same fraction. That is the specific complaint; the sweep behind it is the general one. Measured on the live client, per panel: **SeasonPanel carries 88 `Color.Locked` surfaces and a `TrackTray` at rgb(104,98,138), luminance 0.41 over 182,000 px² — the darkest surface in the game** on a screen where every other panel is light; MasteryPanel 20 greys plus one surface at luminance **0.12**; RobuxPanel 9; Inventory / Character / Egg 4 each. Add the flat ones a colour probe cannot see: ZonesPanel's rows are desaturated mud where `GameConfig` holds a real colour per zone, ShopFrame's four DNA tiles are one green with a **dead grey rectangle** where a fourth would go, the Journal's preview pane is a large empty grey slab, the Auras rows are white on white with the **"Wearing" button painted in the refusal grey**, and Inventory has a wide empty white band beside its tabs. One rule fixes most of it and it is already in the kit: **claimed / mastered / owned / passed is `DoneShade`, not `Locked`** | Open every panel in turn: no panel has two readouts of one number, no panel has a surface darker than the rest of the game, and nothing that was *earned* is painted the colour of a refusal | **done 2026-08-19, all three clauses measured live with every panel open at once.** Most of the Season work was already on disk — it came back in **19.0**, in the Studio session that had never been committed — so this row was half built and unverified rather than unstarted, which a lint could not have told anyone. **Clause 1, two readouts of one number:** the 30-segment rail is binary now, measured **10 full / 20 empty / 0 PARTIAL**; a partial segment is the sliding edge she photographed, and there is not one on the board. **Clause 2, the dark slab:** `TrackTray` is still 182,468 px² and is now `BackgroundTransparency = 1.00`, so the rgb(104,98,138) luminance-0.41 rectangle is gone. Every remaining “dark” surface a colour probe reports is `ShadowBody` — 18.5's drop-shadow sprite, which is supposed to be dark, and reading it as a fault is the trap in this measurement. **Clause 3 is where the real work was, and a raw `Locked` count cannot see it:** the count says nothing about whether a grey is a *refusal* or a *receipt*. Captioning every locked-grey surface and reading it back found **14 rows saying “✓ Mastered”, 9 buttons saying “OWNED”, and the Auras and Journal “✓ Wearing” buttons** — every one of them a thing the player had earned or paid for, painted in the swatch that means *you cannot have this*. Four call sites now use `DoneShade` of their own hue, the same three-state split 18.3 made on the Daily board: **MasteryPanel 20 → 5 greys, RobuxPanel 9 → 0**, and a mastered row / OWNED button now reads **rgb(214,238,224)**. **Final sweep over the whole HUD with all twenty panels open: 96 locked-grey surfaces remain and ZERO of them is captioned as earned** — SeasonPanel's 73 are all `🔒 Lv.N` / `🔒 Premium`, CharacterPanel's four are unowned event skins, which is exactly what grey is for. Captures of Stage Mastery and the pet cards |
| 18.7 | `[x]` | <!-- found 2026-08-16 while sweeping for 18.6 --> **The DNA Splicer is the one screen built by another file, and it shows.** `SplicerUI` is its own `ScreenGui`, so every pass over `MainUI` has missed it: its odds table is printed on a **`PanelBlue` slab at luminance 0.59** with seven rarity colours competing against the card instead of against each other (Common sinks into it, Secret is near-black on mid-blue), the percentages are **cream on that same card**, its close button is the only rounded **square** with a capital **X** in a game of red discs with a ✕, the worn-mutation card is a fixed lavender with the rarity printed as coloured *text* on it (Mythic's red on lavender), and the pity meter is a 16 px stripe carrying no number at all | Open the Splicer: seven rarities legible as a ladder, the card wearing what you are wearing, and a meter that says how far along it is | **done — verified 2026-08-19 by capture, and it needed no code either.** The rebuild came back in **19.0** with the rest of the uncommitted Studio session; this row was shipped and unverified. All four faults measured gone on the live screen: the odds card is **rgb(240,243,252)**, not the `PanelBlue` slab at luminance 0.59, and the percentages are **rgb(48,38,66) dark ink** on it rather than cream; the seven rarities read as a ladder because each now has a real **swatch chip** — Common rgb(200,200,200) through Godly rgb(255,240,150) — each with the kit's 3 px ink outline, so a pale rarity is bounded rather than sunk; the close button is a **42×42 red disc, rgb(245,68,85)**, matching every other close in the game rather than being the one rounded square with a capital X; the worn card is the **rarity's own colour** (Mythic red) carrying *“Wearing Mythic — x1.50 income, +8% speed”*, not a fixed lavender with the rarity printed as coloured text on it; and the pity meter is a 24 px bar reading **“0 / 10”** under the caption *“Charged roll in 10”*, where it was a 16 px stripe carrying no number. **A measurement trap worth recording:** the rarity NAME labels sit at 1.04–2.39 contrast against the card and a naive contrast pass calls five of seven illegible — they each carry a **4 px rgb(26,18,36) halo**, which is rule 7's “ink and halo are ONE decision”, and the capture shows all seven reading cleanly. `Secret` is the correct exception: near-black ink, halo thickness **0** |
| 18.8 | `[x]` | <!-- root cause of 17.13, found 2026-08-16 by measuring the pet cards she photographed --> **A caption handed to a shell disappears, and this is the fourth time it has shipped.** `EnchantChip` on every pet card is a `TextLabel` at ZIndex 32 that `styleCard` shells with `InnerBody` at **33** and `Gloss` at **34** — so its text is painted over and the card shows a blank coloured pill, which is exactly the screenshot behind **17.13**. Every property reads correct: `.Text` is right, `TextFits` is **true**, `TextBounds` 66x15 in an 84x28 box. 16.6 fixed this for the two Inventory tabs, 15.37 for the Journal disc, and each fix was local. The mechanism fix is one function — mirror the caption into a `Label` child above the body, keep the host's string (so call sites that read it back still work) and stop the host drawing it — called from `applyShell` for the kit's own widgets and from `MainUI.styleCard` for the 77 of 78 surfaces the kit does not build | Sweep every text run in the HUD for a caption whose own ZIndex is at or below an opaque `InnerBody`: zero, and the pet cards say what their buttons do | **done — verified 2026-08-19, and it needed no code.** The mechanism fix this row asks for already existed on disk: `UITheme.mirrorText` plus its two callers, `applyShell` (for the widgets the kit builds) and `UIKit.styleCard` (for the 77 of 78 surfaces it does not). Both came back in **19.0** — written in the uncommitted Studio session — so the row was shipped and unverified, and only a live sweep could say which. **The sweep, with all twenty panels open: 1,939 visible `TextLabel`s, ZERO with a caption at or below an opaque `InnerBody`.** The four hosts the row names read exactly as designed — `EnchantChip` hosts sit at `TextTransparency = 1.00` (still holding their string, so call sites that read `.Text` back keep working) with a mirror `Label` at **ZIndex 36 above the `InnerBody` at 33**, carrying the same string and visible. **Capture of the Pets panel shows nine chips reading “Radiant”, “Prismatic”, “Fierce” and “Keen”** — the same cards 17.13 was photographed on as blank coloured pills |

| 18.9 | `[x]` | <!-- coded + pushed + verified live 2026-08-17 --> **`ZoneBuilder` is eight files.** 9,281 lines and **198 of Luau's 200 top-level registers** — two declarations from not compiling, in the file where crossing the cap does not break a panel but stops the world building at all. Cut into `ZoneKit` (the build vocabulary), `ScatterKit` (where a prop may stand), `VillageKit` (what a village is made of), `ZoneGate`, `ZoneTerrain` (which holds `buildValleySide`, ~1,270 lines, the largest function in the game), `BiomeDecor` (the four layers + all twenty zone builders), `EggPlaza` and `EventArena` | Every module requires cleanly and `ZoneBuilder.Build()` still builds the same world | **2,472 lines and 87 registers, no call site outside the moved text changed.** Every cut is a script under `tools/splits/` that asserts its boundaries before slicing, refuses to run twice, and checks BOTH directions afterwards — that no moved name is still spelled in the host (a survivor is a nil global, and a nil global in a builder is a silent hole in the world, not an error), and that no name that stayed behind is reached for in the moved text. `tools/luaregs.py` is new and is what measured this: every scan before it counted `local` **lines**, and Luau counts **names**, which is the whole difference between the 190 this split was planned against and the 198 it was at. It reproduces 169 at `1c9ec1e` and 87 today. **The plan in `docs/SPLIT.md` §6 was wrong in two ways worth keeping:** the enabler was invisible to a leaf-shaped plan (`ScatterKit` is not a leaf and is 253 lines, and nothing could move until it existed, because `scatterPoint` is read 82 times inside the biome layer alone and `darken`/`lighten` another 32), and four of the listed leaves were **one** leaf (every layer verb had exactly one caller, which had one set of callers, read by one line in `Build()`) — so a defines/escapes/imports scan over a line range is cheaper than any of the cuts and should come first. Verified in Studio with fresh clones of all nine modules: every one requires cleanly and `ZoneBuilder`'s public surface is unchanged at five functions. `luastruct` + `luascope` clean on 100 files, `luaremotes` 53/53, `luanames` unchanged at 13, all 100 byte-identical to Studio. **⚠️ The geometry census is NOT taken** — see 18.10 |
| 18.10 | `[x]` | <!-- opened 2026-08-17 by 18.9, which cannot close it itself --> **The split owes a rebuild, and a lint is not evidence about a picture.** `BUILD_VERSION` was deliberately not bumped (same builders, same call order, same values), so the standing world is still the pre-split one and nothing has yet proved the new modules build the same geometry. `docs/SPLIT.md` §6 step 4 lists what to measure and why each number is the one a broken cut would break: `TerraceTop` 784/784 solid, `ValleyRock` 410/410 with `ValleyRockBase` 0/410, the Celestial throne at exactly cx − 130 and the Volcano cone at cx − 150 (a broken placement frame is invisible to every lint here and puts a mountain in the middle of a village), and **AbsolutePlane's floor at rgb(204,204,204) Marble, which is the most load-bearing of them now** — `GROUND_MATERIAL` reaches `ZoneTerrain` as an argument rather than as a table it holds, so an empty one shows there first. Plus a plaza with three shells on their podiums, a walkable gate, and the arena standing with its way home. **The clone trap now has eight ways to fire:** a fresh clone of `ZoneBuilder` does not give you fresh kits — re-parent clones of all of them or you are judging the old ones | A full rebuild in Edit, the census above, and captures of a plaza, a gate and the arena | **done 2026-08-19: the census reproduces, and the eight modules build the same world.** The world rebuilds on every Play in this session (`stamp 135 -> 136`), so every number below is off a world built entirely by the post-split builders. **21 zones, 107,350 descendants.** The two frame-built structures land on their offsets **exactly**, which is the number a broken placement frame would break and no lint can see: the Celestial throne at **cx − 130.0** on both `ThroneSeat` and `ThroneBack`, the Volcano cone at **cx − 150.0** across all 7 parts. Solidity, which is what 11.22 and 11.23 fixed: **`TerraceTop` 779 / 779 solid**, **`ValleyRock` 410 / 410 solid with `ValleyRockBase` 0 / 408**, `GroundRock` 53/53, `BenchSeat` 120/120, `StallCrate` 80/80 — the last three matching the recorded counts to the part. 17.7's floor holds with its control: **AbsolutePlane 18% of parts over 0.85 luminance against Forest's 14%**, i.e. still the brighter of the two. The plaza has its **3 egg shells on 18 podium parts**, and the arena gate stands **across** its approach (X spread 336, Z spread 24), which is the yaw that row fixed. **Read the small variances as scatter, not as drift:** 18.9 recorded 784 `TerraceTop`, 104,896 descendants and 20% / 17% brightness against 779, 107,350 and 18% / 14% here, because Forest's props are `math.random`-placed and re-rolled on every rebuild — 19.9 is the row that paid for learning that. **The claims that must be exact are the ones about structure, and every one of those is exact.** No audit warning on any boot |
| 18.11 | `[x]` | <!-- Kristina 2026-08-17, with a screenshot of the new tile design: "ovo ovde sad treba da zamenis za shop rebith i ostalo sto vec postoji... treba samo da ga ubacis umesto starog i napravis funkcionalno" --> **The new card-panel design existed only in Studio, and three of its panels were mockups wired over working features.** `StarterPlayerScripts.UIComponents` had never been pushed to disk. `ShopPanel` sold "OP Magic" at R$899 and "GM Commond" at R$1999 — neither product is in `GameConfig.RobuxProducts`, neither has a `productId`, and its buy button called `print(...)`, on the screen the game earns on. `ZonePanel` offered three "Stage N" cards and printed, against twenty-one real zones. `PotionsPanel` read the real config and printed. Plus an "HD POTIONS HOOK" appended to MainUI that cloned any button whose text contained "Potions", **destroyed the original along with its real handler**, and rewired the clone to the mockup | Open Store, Teleport, Rebirth and Potions: each lists the real thing and its button does what it says | **all four functional and verified live, not by lint.** Wired to the same doors the old HUD uses — `TeleportToZone`, `UsePotion`, `PromptRobuxPurchase`, `Rebirth`; `RebirthPanel` is new, she asked for it. **The proof is the potion loop:** clicking USE on the real Medium DNA card took the bag **7 → 6**, applied a **x3 DNA boost with 584 s left**, and the card re-drew itself to "x6 held / Active — 9:44 left" while the HUD's own potion timer independently showed "x3 DNA 9:34". Four captures. **The builder could only ever DRAW a panel once**, which is the root of all of it: `AddCard` returns a handle now (without one, "this zone is unlocked" costs a full rebuild, and a rebuild every three seconds throws the scroll position away), plus `Clear()` for the one list whose length changes, `SetOpen`/`IsOpen` so a hidden panel can decline to refresh, and an `OnRefresh` hook so a panel is never shown with the numbers it was built with. `PlayerData` is new: one `DataUpdate` subscription instead of four, with `Get()` a **function**, because the table is replaced on every push and a cached copy would be frozen at the first payload — or nil, which is exactly the window a new player is looking at the screen in. **Four drawing bugs only the captures found, each caused by the fix before it:** `CanvasSize` read `AbsoluteContentSize.Y` in the same frame the card was created in (stale, so every list was one card short of scrollable); adding `ClipsDescendants` for the stroke that painted over the header then **sliced the right-hand outline off every card**, because a 620 px card in a 630 px frame with a 6 px scrollbar centres to 2 px a side against a 4 px stroke; `Image = ""` is not an invisible `ImageLabel` but a 120 px hole the text is indented past; and a fixed-width title ran under the close button. **One panel was on the wrong tile:** the "Shop" tile opens the DNA *upgrades* panel, so the Robux store moved to the "Robux" tile where it has always belonged. Potions had no tile at all — a tab inside Inventory, two presses away for the one consumable used mid-fight — and now has slot 5 of the left column, built like the Auras tile: **held by nobody**, found back as `screenGui.PotionsButton`, because one more top-level local deletes the HUD. Every require is inside its handler for the same reason, and MainUI is still at **166 of 200** registers. Her icon art was adopted rather than re-sourced — the basket, rebirth arrows and bag came out of five prototype `Test*ButtonGUI` ScreenGuis, which were **drawn over every panel in the game** (the first Teleport capture has "Bag", "Rebirth", "FREE!" and "Pets" sitting on top of the Forest card) and are now disabled, not deleted |
| 18.12 | `[x]` | <!-- opened 2026-08-17 alongside 18.11, her call --> **The old panels are still built and nothing opens them.** `zonesPanel`, `rebirthPanel` and `robuxPanel` are still constructed and still refreshed on every `DataUpdate`; only their buttons were repointed. Deliberate for one commit so the old and new can be compared side by side in Play. Deleting them takes ~600 lines and some registers off MainUI. **What the new panels do NOT yet cover, and must before the old ones go:** the Robux panel's game-pass column (a different door, `PromptGamePassPurchase`, and a different question — "do I own it" rather than "what does it cost"), and per-zone / per-product card art, which does not exist yet — both lists currently fall back to the collapsed no-icon gutter | Confirm the new panel covers everything the old one did, then delete the old one | **done and verified live 2026-08-20. 437 lines and 21.6 KB off `MainUI`; 160 -> 144 top-level registers, so the headroom under Luau's cap went 40 -> 56.** Measured in Play: `ZonesPanel`, `RebirthPanel` and `RobuxPanel` all absent from the live `PlayerGui`, and the HUD is **10,360 descendants** against the 12,316 this row measured before -- the 1,956 it named, exactly. `ZonePanel` draws its 21 cards and `RebirthPanel` reads *REBIRTH -- COMPLETE* with four `DONE` rungs against a save at `Rebirths=4 / StageIndex=20`, which is what the deleted panels said. **THE DELETION WAS NOT THE WORK; THE FOUR DOORS WERE.** `robuxPanel` was orphaned *from the Robux tile* in 18.11, but four other things still opened it: the `+` on the DNA pill, the `+` on the Diamond pill, the egg panel's Auto Hatch button when the pass is not owned, and the in-world *Robux Shop* counter in every Upgrade Emporium (`GameConfig.ShopKinds.upgrades`). **The game was shipping two different Robux stores and which one a player saw depended on which button they pressed** -- and it corrects 19.12's framing: the passes were unreachable from the TILE, not from the game, because two of those four doors landed on the pass tab. All four now call one `hudRefs.openStore(passKey)` (a table field, so no register), which is `SetOpen(true)` rather than `Toggle` because every one of them is a player asking for the store by name. `passKey` is a SCROLL HINT only -- new `ShopPanel.Focus`, deferred one frame because `Builder.SetOpen` rewinds the canvas *after* its refresh, so a same-tick scroll is overwritten. Measured: `Focus("Pass_AutoHatch")` puts that card **12.0 px from the top of a 365 px window** at canvas 2937 of 3668, captured. **The one thing that would have broken silently:** `HUD/RebirthBeacon` is a HUD element, not a panel element, and the only thing that ever told it whether to shine was the deleted panel's refresh -- so 115 lines of `refreshRebirthPanel` collapse to a 5-line `hudRefs.refreshRebirthReady` asking `CanRebirthNow` and nothing else. Verified: beacon `Enabled = false` and `CanRebirthNow` false-with-`"done"` on the same save, **and the four panels refreshed on the lines AFTER that call all hold live values** (Mastery's buy buttons read the owned tick), which is the proof the call ran rather than threw. Pass ownership re-measured through the new file: **9/9 OWNED** at rgb(214,238,224) with `AutoButtonColor = false`, **0/9** with `Passes = {}` -- 19.12's reading, unchanged. Lints: `luascope` clean, `luanames` 13/13 baseline, `luaremotes` 58 remotes all paired. All four changed files pushed and re-hashed byte-identical to `src/`, `loadstring` clean. **NOT VERIFIED:** no real mouse press reached any of the four doors -- the probe VM does not share the client's module cache (its stub `getData` survived four seconds of payloads), so the store was driven through a fresh `require` of the same source rather than through `hudRefs.openStore` itself; and the beacon's *true* branch was not re-measured, because the test save is at 4/4 and a save write is refused by the harness. `HUD/ProductTiles`, `HUD/PassShop` and `HUD/RebirthRungs` are now required by nothing and were LEFT ON DISK -- unrequired ModuleScripts cost no runtime, and they are what `ShopPanel` was ported from. Per-zone / per-product card art is still missing and both new lists still fall back to the collapsed no-icon gutter |
...
| 19.1 | `[~]` | **Banked Relic Chests can never be opened — this is paid content that pays nothing.** The Lucky Wheel's `relic` segment (weight **5**, so ~1 in 20 of every 99 R$ spin, 25-shard spin and free daily spin) calls `RelicService.GiveChest`, which does `data.RelicChests += 1`, and `SpinReveal` tells the player *"1 Relic Chest — open it in the Forge."* **`HandleOpenChest`'s `"banked"` branch is sent by nothing**: the only two `OpenRelicChest:FireServer` calls send `"free"` and `"diamonds"`. The panel never reads or displays the count, and `RelicChests` is not in `defaultData()`. Worse, an unknown `source` falls through to the **free-timer** branch, so the only players who can spend these are the ones crafting the call by hand | Land the relic segment, see a count in the Forge, open it, see the count fall and a relic arrive. Control: the free timer must NOT have fired | **coded + pushed 2026-08-19; both halves measured live, the success path still owed.** **A banked chest is not a third door, it is the free button already open** — the row is 48 px and a third button would crowd it, so the free button becomes the banked one while any are held: same slot, gold instead of green, and it sends `"banked"`. **Spending the bank first is what makes that safe**: the free timer keeps running underneath, so opening a banked chest at any moment throws nothing away. `RelicChests = 0` is now in `defaultData()`, which is why the field existed for weeks without one — the service incremented a number `Load`'s backfill had never been told about. **Server half:** firing the real `OpenRelicChest("banked")` from a client with 0 held returned the branch's own refusal, *You have no Relic Chests!* — so the branch is reachable from a client for the first time — and **`LastRelicChest` did not move (1786982096 → 1786982096) and `RelicChestsOpened` stayed 1**, which is the control that matters: the free chest was READY at that moment, so a fallthrough would have spent it. **Client half:** with a synthetic payload carrying `RelicChests = 3` pushed at 0.15 s, the live button in the live PlayerGui read `Text = "OPEN CHEST (3)"`, enabled, gradient **(255,226,130)→(228,158,20)** — the authored gold pair to the byte. That caption is produced only inside `if bankedChests > 0`, so it is a direct reading of the upvalue the callback branches on. **Control:** when the push window closed and the real save returned, the same button went back to **(120,255,170)→(20,200,100)** and *FREE CHEST* in one second. The save was verified untouched afterwards — every relic field back at baseline. **NOT VERIFIED: a chest actually opening.** Two routes were tried and both are closed to an agent here — a DataStore write to plant the fixture is refused by the harness, and MCP mouse input does not reach this Play session at all (three attempts, including a real `moveTo` + down/up on a visible, hit-testable button; the server-side door heard nothing, and `VirtualInputManager` needs a capability this thread lacks). It closes on one real wheel spin landing the relic segment |
| 19.2 | `[x]` | **One error in the spin reveal kills the Lucky Wheel for the rest of the session while the server keeps charging.** `draining` is cleared only on the drain loop's normal exit, and the five builders run **outside** the `pcall` — which wraps the animation only, inside a different `task.spawn`. Any error there strands `draining == true`: every later `SpinResult` is queued and the drain returns immediately. The player pays Robux, shards or their free daily, the server grants normally, and **the client shows nothing at all** — the notify fallback was deliberately removed. Unrecoverable without a rejoin | Throw on purpose inside `buildWheel`, then fire three more results and see all three render | **done 2026-08-19, verified by fault injection.** `playChain` is now a protected door around a `buildAndPlay` that holds the old body, so a builder that throws returns `false, unbuildable` instead of unwinding the drain loop; `draining` is cleared after a `pcall` around the whole `while`, i.e. on every exit rather than on the one where the loop ended normally; and the retry both **peeks instead of popping** (a failure has nothing to put back, so it cannot lose a payload by forgetting to) and is bounded at `MAX_BUILD_ATTEMPTS = 8`, because a payload that can never build would otherwise hold every later spin behind it — the same wedge by a slower road. **The test found a second leak of the same shape and it would have survived the fix.** `error()` injected at the top of `buildWheel`, four payloads fired 1.2 s apart: the first pass gave **1 × *it threw while building* and 2 × *still busy after 8 attempts*** — the loop had survived, which was the point, but `buildShell` sets `busy` before any builder runs and only `shell.finish` clears it, so the throw left it to the `task.delay(SHELL_MAX_LIFE, ...)` net **67 seconds** later, with a dimmed, blurred, empty shell over the game the whole time. `pendingShell` now hands the half-built shell back from the error branch, and the same four payloads give **4 × *it threw while building* and zero *still busy***. **Control, real builders restored:** two payloads 0.3 s apart drew and tore down cleanly and in order — shell added t=3335.74, removed 3344.35, added 3344.47, removed 3353.07 — so the queue still serialises and the second wheel waited 0.12 s for the first. Studio re-verified byte-identical to `src/` afterwards |
| 19.3 | `[x]` | **The new-player tutorial starts up to three seconds late.** `FirstJoin` falls back to `Remotes.GetData:InvokeServer()` and **there is no `RemoteFunction` and no `OnServerInvoke` anywhere in the game** — `luaremotes` finds it, and it is dead code that also blocks the thread for 10 s. What it existed to catch is real: if `PushToClient` lands before `FirstJoin` reaches its `DataUpdate` connect — after 7 `WaitForChild`s, 5 `require`s and a ScreenGui plus 8 chevron Parts — `state.data` stays nil, `runGuide()` never fires, and the pan, banner, arrow and trail never appear. It self-heals only on the next push. Fix it the way the rest of the client already works (a re-push, or a `StringValue` like `LiveEvents`), not with a new remote | Win the race on purpose and see the guide run; `luaremotes` clean | **done 2026-08-19, and the severity in this row was WRONG — corrected here rather than quietly.** “Silently never runs” is not what happens: `DNAService` runs a periodic sync that pushes to every player every **3 s**, so a listener that missed the join payload catches the next one and the guide starts late rather than not at all. `runGuide` is also guarded by `state.running`, so those repeats do not re-enter it. What is left is still worth closing — three seconds is a very long time to spend at the start of a first session, and it is a HUD with no numbers in it — but it is a delay, not a loss. **Fixed on the server, where it is fixed for every listener at once** rather than in the one client that noticed: `PlayerAdded` now pushes three times over the first ~1.5 s instead of once. Two extra payloads per join, no new remote, and `player.Parent` is re-checked before each so a player who leaves inside the window is not fired at. **Measured by wrapping the LIVE `PushToClient` from a real `Script`** — which shares `ServerMain`'s module cache, unlike `execute_luau`, whose `require` hands back a fresh copy — and logging the caller of every call: `PlayerDataService` at **+0.46 s, +1.87 s and +3.03 s** where it previously appeared once, with `DNAService`'s first periodic push not arriving until **+2.75 s**. (The spacing is wider than the authored 0.5/1.0 because Studio is still building the world at that moment; the count is the claim.) The dead `GetData` block is gone with it — there is no `RemoteFunction` and no `OnServerInvoke` anywhere in the game, so what it actually did was hold the main thread in `WaitForChild("GetData", 10)` for ten seconds of a new player's first minute. `luaremotes` now reports **58 remotes, every one with a speaker and a listener** |
| 19.4 | `[x]` | **`CardKit.Button` throws the first time a colourless button is re-enabled.** `Card`/`Button` default a missing `opts.colors` at construction but `baseColors` captures the **raw** value, so the button renders, greys correctly on `SetEnabled(false)`, and then indexes nil in `SetGradient` on the way back. All six current callers pass `colors`; this is a brand-new kit being adopted across three files, and disabled→enabled is the path the next caller will take. | Build a button with no `colors`, disable it, enable it | **done 2026-08-19, verified live.** `baseColors` now takes the same `{ WHITE, WHITE }` default the gradient took two lines above it. Measured on a real `CardKit.Button` built with no `colors`: white **(255,255,255)→(255,255,255)** on build, grey **(178,178,190)→(128,128,142)** on `SetEnabled(false)`, and **back to white without throwing** on `SetEnabled(true)`. Control, a button that does pass colours: greys to the same pair and returns to its authored gold **(255,226,130)→(228,158,20)** — unchanged by the fix. **The `Pill` half of this row was withdrawn: it was never a bug.** `opts.transparency or 0.18` was reported as swallowing an explicit `0`, which is JavaScript's rule; in Lua only `nil` and `false` are falsy, so `0 or 0.18` is `0` and the line was always correct |
| 19.5 | `[x]` | **Two `FirstJoin` faults that only show on the device a new player is actually on.** `findEvolveButton()` does a **recursive** `FindFirstChild` over the whole HUD **every Heartbeat** while the evolve step is live — hoist it and re-resolve only when nil or reparented. And `marker` (a BillboardGui) and `pointHL` (a Highlight) are parented to the ScreenGui but are not `GuiObject`s, so `gui.Enabled = false` does not hide them: open any panel and the creature stays outlined with a beacon over it, behind the panel | A capture with the Store open showing no outline and no beacon | **done 2026-08-19, and the measurement moved the fix onto the other half of the problem.** The first attempt cached the button on a hit — which turns out to be worth nothing. Timed on the live HUD at **12,317 descendants**: a recursive `FindFirstChild` that HITS costs **0.0004 ms**, because it stops the moment it matches. One that **MISSES** walks the whole tree: **0.1645 ms per frame, 9.8 ms every second at 60 fps** — and a miss is not the rare case, it is every frame for as long as the HUD is still building, which on a first join is precisely when this runs. So the shipped version caches **both answers**: a hit until the button is destroyed or reparented (a HUD rebuild is still picked up), a miss for 0.25 s. Measured on the shipped shape: 120 frames in 2.0 s did **8 walks instead of 120**, **0.65 ms/s against 9.78 — 93% removed**. **The beacon and outline:** `gui.Enabled = false` governs GuiObjects, and a `BillboardGui` is its own LayerCollector with its own `Enabled` while a `Highlight` renders off its `Adornee` and ignores its parent entirely, so opening the Store left a creature outlined in yellow with a beacon over it, behind the panel. `pointAt` now reads `trailSuspended` for the same reason `updateTrail` already did — **read there and not in the watcher, because the guide loop calls `pointAt` every frame and would switch back on whatever the watcher switched off**. Measured with the guide driven live by a synthetic `TutorialDone = false` payload: closed → `marker=true highlight=true adornee=TerraceRamp`, open → `false / false / adornee=nil`, closed → `true / true` again, twice, and the second re-acquisition picked a **different** adornee (`Body`), which is the proof it is re-resolving rather than restoring. **Control that mattered:** the first run showed the marker not coming back, and that was the climb beat having ended on its own timer rather than a regression — re-run on the `fight` beat, where the loop points every frame, and it returns every time |
| 19.6 | `[x]` | **The Gemini compaction of `FirstJoin` dropped 391 lines, almost all of them the design comments this project keeps on purpose.** 19.0 proved disk was the descendant and pushed it, so the running file is right and this is documentation debt, not a defect. The 915-line Studio copy is preserved at `git show ea70c71:tools/_studio_pull/FirstJoin.client.lua` and is the source to restore from | The comments are back and the file still hashes identical to Studio after the push | **done and verified live 2026-08-20. 674 → 1,042 lines, 60 → 338 comment lines**, pushed and re-hashed byte-identical to `src/` (51,130 / 888389944) with `loadstring` clean. **A MERGE, NOT A REVERT** — 19.0 established the compacted copy was the descendant, so the restoration went ONTO it and every number quoted in a restored comment was re-read off the code beside it. Four were wrong if copied back verbatim and were corrected: headline/subline 26/18 px → **24/17**, the arrow's overlap argument now names XP rather than DNA (`stepFor` tests XP alone), the pips are a vertical **column** not a horizontal row, and the arrow is 76 px not 64. Three blocks are new rather than restored, because 18.23 added things the old file had no note for: the pulsing evolve halo, the sound cue, and the marker's bob. **THIS ROW'S PREMISE WAS WRONG AND THAT IS THE FINDING.** It says "documentation debt, not a defect". The compaction also deleted **two guards** from `runClimbBeat`, and a code-only diff (comments and blank lines stripped from both revisions) shows they are the ONLY executable difference between the compacted file and this one. `if not ramp then return end` is a live defect: without it the fourth beat shows *“Climb to the next terrace — Tougher creatures, higher XP drops”* for a full seven seconds wherever the player is standing, and `pointAt(nil)` then correctly draws no marker and no trail — an instruction with no target. **Measured, so it is not theoretical: the EventArena at (0, -4, 1400) is 1,006 studs from the nearest of the 124 `TerraceRamp`s, past `nearestRamp`'s own 900-stud cut-off.** A player whose first evolve lands in the arena gets exactly that. `state.climbShown` was restored too, as insurance rather than a fix — today's only caller cannot fire twice. **Live control:** the guide still builds all five pieces (`Banner`, `Arrow`, `ButtonHalo`, `GuideMarker`, `GuideHighlight`), `DisplayOrder = 110`, `IgnoreGuiInset = false`, 8 pooled chevrons with `MeshType = Pyramid` and `CanQuery`/`CanTouch` off, 3 pips, and the nearest ramp to spawn is 514 studs so the restored guard passes on the normal path. Lints: `luastruct` and `luascope` clean, `luanames` 13/13 baseline (`runGuide` moved 371 → 869, `src/SYNC.md` updated). **NOT VERIFIED:** the guard's STOP branch was not exercised by standing a player in the arena and evolving — it is reasoned from the 1,006-stud measurement and four lines of code, not watched |
| 19.7 | `[x]` | **`DailyPanel` fires `BuyResource` and no server listens** (`luaremotes`, second finding). The panel is required by nothing, so this is dead code recovered from Studio in 19.0 rather than a live break — decide whether the panel is finished or deleted, and do not leave a third daily surface beside `DailyRewardsPanel` and the old one | `luaremotes` clean | **done 2026-08-19: deleted, from disk and from the place.** It is the shape 18.11 already caught once — a mockup wired over a working feature — and it had three separate wrong wirings, not one: the button captioned **FREE SPIN** fires `ClaimDailyReward`, which is the daily reward and not a spin; **all seven day cards** fire that same remote with no day argument, so every card does the same thing; and the cards print `reward.dna` raw, which in an exponential economy is the Phase 1a bug this roadmap opened with. Nothing required it. **`DailyRewardsPanel` was kept and is the opposite case** — 736 lines that read the streak and the claimed state, quote `GameConfig.SpinCostShards` because that is what the server charges, and list unredeemed codes. It is real work that is simply not wired into MainUI yet, and its line 621 already computes a **looping** reward index, which is half of what row 21.2 asks for. `luaremotes`: **58 remotes, all paired** |
| 19.8 | `[x]` | **Finish the Phase 18 grey-UI pass**, because the UI is the first thing a soft-launch cohort sees: rows **18.6**, **18.7**, **18.8**, **18.10** and **18.12** are all still open. 18.12 must move the game-pass column into `ShopPanel` *before* the old panels are deleted — the pass storefront lives only in the old one | Each row's own check | **closed 2026-08-20 by 18.12, the last of the five.** 18.6 / 18.7 / 18.8 / 18.10 closed on 2026-08-17--19; 18.12 closed today. The precondition this row wrote down was right and it had already failed when it was written -- see 19.12, which had to be opened to fix it, and 18.12's own note on the four doors that were still open after that |
| 19.9 | `[x]` | **The Splicer is 260 studs from where it is meant to be.** Live console this session: `preferred spot was occupied; machine moved 260 studs to (120, 30)`. It is a machine the player has to find, and a placement search only knows the world that existed when it ran | The machine stands at its authored spot with a capture | **done 2026-08-19, and the cause was not the search.** The occupancy test excluded only the machine, so **creatures counted as obstructions** — measured on a live server, the only blockers on the preferred spot were two `body_geom` parts, i.e. one mob mid-walk. That is why it landed somewhere different every boot: (120, 30), (328, 498), (-192, 290), (380, 290), 221–312 studs out. Creatures are excluded by folder now and the test also requires **`Anchored`**, which is the general form of the same rule: a part that can move is a fact about this instant, not about the place. **That alone did not fix it**, and the next boot proved it — a grid scan of the whole plaza deck at the machine's real 60×60 footprint found **23 clear spots and every one on the WEST verge**; the east side was solid forest props from x = 40 to 180, and the authored point genuinely was under a `ForestTree` and two `PropBOULDER`s. Forest's props are `math.random`-placed and re-rolled with the world, so no single coordinate is ever *the* clear one. **`PREFERRED` is a LIST now, the answer HubPlaza already reached for the same reason** (its `PHOTO_SPOTS`): four authored choices tried in order, with the ring search demoted to the floor under them, and a `print` rather than a `warn` when a later authored spot is used — that is the list working, not a fault. **Then the capture found the bug the tests could not**, which is the whole reason this row's check demands one: the spot it picked, (-72, 312), passed everything in the file and stood **11.4 studs through the Forest arrival board**. `ArrivalSignBoard` is `CanCollide = false`, like every board, banner and plank surface in the game, so no occupancy test could ever have objected. Signage is now rejected **by shape** — anchored, raised above y = 8 (because “Board” and “Plank” are also what the deck underfoot is made of), compared in plan view — which is 12.13's sightline rule one step further out and needs no names or coordinates. The spot list was re-probed against that rule and two entries were dropped. **Result: five boots, each rebuilding the world; every one landed on an authored spot, zero warns, zero signage intersections, nearest signage 71.6 studs.** `MACHINE_VERSION` 6 → 8 — without the bump a played world keeps the machine it already has and the whole fix is a silent no-op. Capture taken |
| 19.10 | `[x]` | **Two documents actively lie about trading and will mislead the next cold agent.** `TradeService`'s own header and this file's Phase 8 preamble both say nothing can reach it. `ServerMain` requires it, calls `Init()`, and all seven remotes exist; row 8.6 is `[x]` further down this same file | Both texts match the code | **done 2026-08-19.** Verified before rewriting rather than assumed: `ServerMain:34` requires the module, `:135` calls `Init()`, and `Init` creates seven remotes and connects the five inbound ones. Both texts now say so, and both keep the history — the old claim was **true when written** (only the server core existed, behind the “not until the game is live” gate) and was simply never updated when publishing the place opened that gate. **The tell that should have caught it: a preamble contradicted its own table**, where 8.6 has been `[x]`, and a preamble is what a cold agent reads first. Both texts also now carry the thing that IS still true — there is no HUD tile, so the only door is knowing to click another player, which is row 21.1. Shipped-and-undiscoverable is a different problem from unwired and the two are worth keeping apart |
...
| 19.12 | `[x]` | <!-- found 2026-08-19 while checking 18.12's precondition --> **THE NINE GAME PASSES HAD NO DOOR. 2,041 R$ of storefront that no player could reach, three weeks from launch.** Not dead code — a live revenue bug, and it was shipped by a row that closed clean: **18.11 repointed the Robux HUD tile at the new `ShopPanel`**, and the pass column lives in `RobuxPanel.PassScroll`, built by `HUD/PassShop` into the OLD panel. Nothing errored, nothing warned, every property still read correct — the panel is simply an orphan, measured on the live client as `Visible = false` with no handler anywhere that opens it. The new file even documented the gap (*“WHAT IS NOT HERE: the game passes… they keep their own panel”*) without anyone checking whether that panel was still reachable. **General rule, and it is the second time this exact shape has bitten this project: repointing a door is a DELETION of everything that was behind the old one.** 18.11 checked that the new panel worked; nothing checked what stopped working. | The nine passes are buyable from the tile a player actually presses, and an owned one says so | **fixed and verified live the same session.** The passes are cards in the new store now, sorted after the products at `LayoutOrder` 1000+ — a pass is a permanent multiplier and a product is a one-off grant, and interleaving the two ladders is what makes a store unreadable. They keep their own door (`PromptGamePassPurchase`, **the key not the id**, so a tampered client can only name a pass that exists) and their own question: an owned pass shows **OWNED** instead of a price. Measured both ways on the live payload: with Studio's all-passes-granted state, **9 of 9 read “OWNED” at rgb(214,238,224) with `AutoButtonColor = false`**; with `Passes = {}` pushed through the same refresh, **0 of 9**, all back to their prices at rgb(120,255,170). Capture of the 2x Speed and 2x XP cards. **One trap this paid for, and it is 18.6's rule with the kit enforcing it:** the builder's `SetEnabled` reads `enabled and (colors or bOpt.Colors) or DISABLED`, so on the disabled branch the colour argument is **unreachable** — passing the OWNED pale-green there paints the refusal grey rgb(178,178,190) instead. An owned pass is therefore kept “enabled” to hold its fill, with `AutoButtonColor` cleared so it does not pretend to depress and an `ownedKeys` guard on the callback; `PassService` re-checks ownership server-side regardless |
| 19.13 | `[x]` | <!-- opened 2026-08-20 by a screenshot: "radi, vrti se, ali su kartice spojene" --> **The Lucky Wheel drew all twelve prizes in one place.** `GuiObject.Rotation` pivots on an element's CENTRE and ignores `AnchorPoint`, so every radial piece on that wheel — twelve pods, twelve pegs, twenty-four bulbs — spun about its own midpoint instead of about the hub, and the twelve prize cards landed as a fan of overlapping rectangles offset up-and-left of the disc. The pegs prove it independently: they came out on a circle of radius half their own length, centred half a length above the hub. Rebuilt as a real pie — every rotating piece is now a full-diameter bar centred on the wheel with its inward half masked by a `UIGradient` transparency step, each sector filled by a fan of nine same-coloured bars, ink dividers over the seams and one ring closing the rim. The win flash is a second fan laid over the first, because a `ColorSequence` cannot be tweened. The flapper pivots on its pin now too, for the same reason. | twelve distinct sectors, evenly spaced, no overlap | Play capture: the wheel renders as twelve labelled wedges; a fired `SpinResult` chain spins, lands on the jackpot and reveals |
| 19.14 | `[x]` | <!-- same screenshot pass: "ovde imaju 2 progress bara, ovaj levo ne treba" --> **The Season Pass still had two readings of one number, and 18.6 kept the wrong one.** That row took the analogue fill off the rail and left the 186×30 `SeasonXP` bar in the level card, arguing the bar was the only one carrying text. The owner's answer on the result: *ovaj levo ne treba*. The rail is the surface a player actually reads the season on — it runs under all sixty rewards — so the bar goes and the level card becomes a readout: a caption, the level as one large numeral, and the XP as text in a `Cloud` well. Gives a register back rather than costing one. | exactly one progress surface on the page | Play capture: `SEASON LEVEL / 10 / 400 / 1500 XP`, no bar in the card, the rail unchanged |
...
### `ServerScriptService.Telemetry` is the only file that calls `AnalyticsService`

Everything below routes through one wrapper, and that is not tidiness. **Roblox's analytics service
records nothing in Studio**: the calls validate their arguments, return, and the dashboard is the
only place a result appears -- 24 hours later, in a published place. A wrapper that only forwarded
would be code no agent could ever verify, and "it compiles" would be the whole check.

So every call is **counted into `Telemetry.Stats` before it is forwarded**, and the forward is
wrapped so an argument the service REJECTED is counted apart from one it accepted. That is what
turns "does the funnel fire" into a question a probe answers in one call.

### The one design decision in this phase: half the economy is batched

`LogEconomyEvent` is a per-transaction call, and two faucets here are not transactions in any useful
sense -- the kill reward (`CreatureService`, which its own comments call "the most frequent event in
the game") and auto-collect (`DNAService`), **which pays every player every second, forever**. Auto-
collect alone is 3,600 events per player per hour; a 30-player server would push 108,000 economy
events an hour for one faucet. That is far past what the service accepts, and useless when it lands.

Two entry points, and the choice between them is about **frequency, never importance**:

- `Telemetry.Economy(...)` -- one event, sent now. Purchases, upgrades, claims, hatches, the rebirth
  wipe. The rows a designer reads one at a time.
- `Telemetry.Accrue(...)` -- added to a per-player bucket keyed by flow/currency/type/sku and flushed
  once a minute, on leave, and on `BindToClose`. **The total is exact** -- nothing is sampled or
  dropped -- only the granularity is coarser, and the ending balance stamped on a flush is the
  balance at flush time, which is where the player actually stands when the aggregate lands.

Getting it backwards is the trap: batching a purchase makes the one event that pays for the game
arrive a minute late and merged with others; sending every kill silently exceeds the limit and loses
**all** of them, the ones that mattered included.

### The funnel only ever runs for a save that was born instrumented

`LogOnboardingFunnelStepEvent` is a **lifetime** funnel. Firing it for a player with a thousand hours
enters them at whatever step they happen to trip first -- a veteran opening an egg is a step-8 event
with no steps 1-7 under it -- and the drop-off curve every later phase is measured against would be
reading a mixture of first sessions and thousandth ones.

So `PlayerDataService.Load` stamps the save: the **fresh** branch writes `data.Funnel = {}` and that
save is followed step by step for life; the **returning** branch writes `{ pre = true }` and is never
logged. `data.Funnel` is deliberately **not** in `defaultData` -- the backfill loop there copies every
default onto every existing save, which is exactly the thing that must not happen to this one field.
The cost is that the funnel is empty the day it ships and fills from the first genuinely new player,
which is what it should measure.

**A finding, and it is about this game rather than about the instrumentation.** Steps 5 and 6 --
first evolve and tutorial done -- are seconds apart, because in Evolution Lab *the tutorial IS the
first evolve*: `DNAService.HandleEvolve` sets `TutorialDone`, and `FirstJoin`'s guide reads that flag
to take itself off screen. Step 6 is still worth having (it is the only step that proves a **client**
reached the end of the sequence rather than the server having written the flag underneath it) but
**the interesting drop-off in this funnel is 4 → 5, not 5 → 6.** Do not read 5 → 6 as retention.

**Two steps can only come from the client, and a client may report only those two.** `loaded` (the
loading screen finished its wipe -- the server knows when it *pushed* the data, not when the client
stopped covering the screen with it, and the gap is up to three seconds of `MIN_SHOW` plus fade) and
`tutorialDone`. Both arrive over `Remotes.TelemetryStep`, which refuses any key outside a two-entry
whitelist: everything else in the funnel is a claim about the world, and a client that could report
a kill could report having finished the funnel on the join frame.

| ID | | Task | Check |
|---|---|---|---|
| 20.0 | `[x]` | **The wrapper** — new `ServerScriptService/Telemetry.lua` (425 lines). `Funnel` / `Economy` / `Accrue` / `Custom` / `SessionEnd` / `Flush`, one `pcall` for the whole file, `Telemetry.Stats` as the test surface, and `Init()` wired **first in `ServerMain`, before `PlayerDataService`** — it reads the leaving player's balance out of `PlayerDataService.Cache` to stamp on a flush, and PDS's own `PlayerRemoving` clears that entry, so connection order is the whole difference between a real ending balance and nothing | Verified in Edit 2026-08-20: all six `AnalyticsEconomyTransactionType` names resolve; `AnalyticsEconomyFlowType.Source/Sink` resolve; the module loads and builds its tables; **funnel steps are 1..8 with no gap and no duplicate**; a nil player, a zero amount and an unknown flow are all refused with `sent = 0`; two `Accrue` calls send nothing; the funnel **dedupes** (two identical calls counted once) and **refuses a `pre = true` save**; a bad player throws inside the wrapper and lands in `Stats.failed` / `Stats.lastError` instead of reaching the caller. `[~]` **only** because a real `Player` has never been passed — see the blocker under the table **THE BLOCKER IS GONE — a real `Player` was passed, 2026-08-20, and `AnalyticsService` accepted every argument this file builds.** One Play session, driven through the live module (see the bridge note below): **29 calls, `failed = 0`, `lastError = nil`**, and the engine narrated each one into the console itself (`AnalyticsService: LogCustomEvent event fired.`, likewise Economy and OnboardingFunnelStep) — independent of this file's own counters. The three API calls were also made RAW, outside the wrapper, with the real `Player`: `LogCustomEvent` / `LogEconomyEvent` / `LogOnboardingFunnelStepEvent` all returned `true` with no error, so argument acceptance is proven at the engine boundary and not merely at ours. **`Telemetry.Stats` cannot be read with a plain `require`** — a probe VM gets a fresh module with zeroed counters while `PlayerDataService.Cache` reads empty on a server that loaded the save perfectly; the tell is that `leaderstats` (written by the real VM) held `DNA = 5.9e19` at the same moment. The route that works is injecting a `Script` at runtime so it runs in the REAL VM and mirrors `Stats` into a `StringValue`. **STILL `[~]`, for a NEW and narrower reason than before:** this row claims `Init()` is ordered first so `flush` can read the leaving player's balance out of `PlayerDataService.Cache` — and that whole leave-time path is defeated by a guard inside this file. See **20.6**, which falsifies it. Everything measured above is the IN-SESSION half |
| 20.1 | `[x]` | **Onboarding funnel** — all 8 steps hooked. 1 `joined` `PlayerDataService.Init` (after the save loads, so a failed read is a kick and not a phantom at the top of the chart) · 2 `loaded` `LoadingScreen`, after `gui:Destroy()` · 3 `firstSwing` `CreatureService.onHit`, **below** the reach/alive/rebirth guards, so a swing the server refused is not a swing · 4 `firstKill` at the kill payout · 5 `firstEvolve` `DNAService.HandleEvolve` · 6 `tutorialDone` `FirstJoin`, at the last beat · 7 `firstZone` `ZoneService`, **inside** the `travel()` guard so a failed transition claims nothing · 8 `firstEgg` both egg doors | Structure verified above. **Still owed: every step non-zero in the dashboard within 24 h of the publish** **Five of the eight steps fired live on 2026-08-20, through the real doors, with a real `Player`:** `firstSwing=1 firstKill=1 firstEvolve=1 firstZone=1 firstEgg=1`, `failed = 0`. Steps 3 and 4 came from a real swing — `Remotes.AutoAttack:FireServer(model)` fired from the **Client** datamodel so it crossed the network into the real handler — at a Critter 20.1 studs away, which went 30 → 0 hp on the first blow. Step 7 from `Remotes.TeleportToZone` (Forest → Desert, hrp confirmed at 1900, 21.5, 490), step 8 from `Remotes.BuyEgg` (`ForestBasicEgg`), step 5 from `DNAService.HandleEvolve` — whose remote handler is literally that one call, so the direct invocation is the same code path. **The funnel had to be un-stamped to see any of it:** the owner's save is a veteran (`Funnel = { pre = true }`) which `Telemetry.Funnel` refuses BY DESIGN, so it was temporarily set to `{}` and put back — the restore was owned by the injected script under a deadline plus `PlayerRemoving` plus `BindToClose`, never by a follow-up call, because the 60 s autosave or the leave-save would otherwise write a veteran into the funnel permanently and poison the exact baseline 20.5 exists to measure. Restored and independently re-read: `pre = true`. **Steps 1 / 2 / 6 were NOT measured** and cannot be on this save — 1 `joined` never fires for a `pre` save, and 2 `loaded` / 6 `tutorialDone` are client-reported and already long past. They need a genuinely new save. Dashboard check unchanged and still owed |
| 20.2 | `[x]` | **Economy events** — **45 call sites across 20 files** — 38 sent immediately, 7 batched. DNA / Diamonds / Shards, both directions. Sources: kills, boss, world boss, auto-collect, offline, daily, spin (+ the fallback segment), free spin, playtime gifts, season pass, event ladder, codes, group chest, like, favourite, photo pad, VIP daily, the Phase 12 mutation refund, and **every Robux product with `Tx.IAP`** so it lands against revenue rather than gameplay income. Sinks: upgrades, diamond upgrades, stage mastery, eggs (single + bulk), enchant, potions, relic chest, relic merge, splice, shard spin, **and the rebirth wipe** — without that last one the supply chart shows DNA minted forever and never destroyed, which is the exact shape of a broken economy in a game whose economy is fine | `luaremotes` OK (60 remotes, all paired), `luastruct` OK, `luascope` OK. **Still owed: both directions appear for all three currencies on the dashboard** **Both directions confirmed live for DNA and Diamonds, 2026-08-20.** Sources measured: `Source/DNA/Gameplay/kill`, `Source/DNA/Gameplay/autoCollect`, `Source/DNA/TimedReward/offline`, `Source/DNA/TimedReward/groupChest`, `Source/Diamonds/Gameplay/kill`, `Source/Diamonds/TimedReward/groupChest`. Sinks measured: `Sink/DNA/Shop/egg:ForestBasicEgg` (500), `Sink/Diamonds/Shop/diamondUpgrade:MegaIncome` (76), `Sink/EvolutionShards/Shop/shardSpin` (25). **The batching works end to end** — the kill and auto-collect rows only ever appeared as aggregates written by the 60 s flush, exactly as designed. **`Source/EvolutionShards` was NOT observed**, and that is a gap in the measurement rather than a fault: the only gameplay shard faucet is a random `RollShardDrop` on RAISED creatures, and five ordinary ground-level kills did not roll one. **The leave-flush is BROKEN — see 20.6.** `Telemetry.Accrue` buckets drained at `PlayerRemoving` send nothing, so whatever accrued since the last 60 s tick is lost on every single departure. Dashboard check unchanged and still owed |
| 20.3 | `[x]` | **Custom events** — `TradeOpened` (on the requester, past every guard) / `TradeCompleted` (once per side, so it is comparable with the open) · `FriendsInServer` (fired once with a **count**, off the join thread, **and fired at zero on purpose** — "joined alone" is the baseline Phase 22 is trying to move, and an event that only exists when the answer is good measures nothing) · `WorldBossContribution` (from the payout loop, so it counts people not bosses) · `MutationRolled` (the roll, not the equip) · `SpinTaken` (value 0 free / 1 shards) · `ChestOpened` (below all three relic-chest branches, plus the group chest) · plus 20.4's four | **Still owed: each fires once in a scripted session** **Three of the seven fired live on 2026-08-20:** `FriendsInServer = 1` (fired at zero, off the join thread, on a solo server — which is the baseline this row says it wants), `ChestOpened = 1` (group chest, via `Remotes.ClaimGroupChest`), `SpinTaken = 1` (paid spin, via `Remotes.SpinWithShards`, value 1). **NOT measured:** `TradeOpened` / `TradeCompleted` (need two clients — see [[evolution-lab-two-client-testing]]), `WorldBossContribution` (needs a world boss), `MutationRolled` (needs a splice). `ClaimFreeSpin` and `ClaimDailyReward` were fired and correctly REFUSED — both had already been claimed on this save today, so `SpinTaken = 0` (the free daily) is still unmeasured |
| 20.4 | `[x]` | **Session-end state** — four separate custom events rather than one with three fields, so each arrives as its own **distribution**: `SessionMinutes`, `SessionEndEvolveBarPct` (numerator `data.XP`, denominator `GetEvolveStep().xpCost` — the exact ratio the HUD draws, so it cannot disagree with what the player saw), `SessionEndClaimable` (a count: daily + free spin), `SessionEndTimers` (potion boosts still running). Every read is defensive — this runs inside `PlayerRemoving`, where the character is already gone and a service can be mid-require | **Still owed: a distribution, not a single number** **FALSIFIED LIVE 2026-08-20 — this feature does not work at all, and none of its four events has ever been sent.** Measured by a hook connected LAST (so it observes what the game's own handlers left behind) which dumped `Telemetry.Stats` from inside `PlayerRemoving` and again at `BindToClose`: `sent = 29`, **identical to the value before the stop**, and `custom` holds only `{ChestOpened, SpinTaken, FriendsInServer}` — no `SessionMinutes`, no `SessionEndEvolveBarPct`, no `SessionEndClaimable`, no `SessionEndTimers`. The cause is **20.6**. **Re-measured 2026-08-20 after the 20.6 patch and the events now ARRIVE:** `SessionMinutes = 1`, `SessionEndClaimable = 1`, `SessionEndTimers = 1`, `sent` 6 → **11**, `failed = 0`, read out of `BindToClose` in the console. `SessionEndEvolveBarPct` is absent and that is CORRECT rather than a fourth failure — the owner's save is `isMax`, which is the branch that deliberately skips it, so proving that one needs a save mid-chain. **`[~]` and not `[x]` for one reason only:** the row's own check asks for a *distribution*, and three events from one session on one save is a point, not a shape. What was owed — that the feature emits at all — is now measured |
| 20.5 | `[ ]` | **Baseline in the first 72 h**, written into this file as the number every later phase is compared against. Blocked until the 4 September publish — there is nothing to read before there are players | D1 / D2–7 / session length / play-through rate recorded |
| 20.6 | `[x]` | <!-- opened 2026-08-20, found by the Play session that closed 20.0's blocker --> **`Telemetry.Custom` and `Telemetry.Economy` both refuse every call made from `PlayerRemoving`, because `player.Parent` is already `nil` there.** Both begin `if not player or not player.Parent then return end`. That guard is right for the in-session calls it was written for and **wrong for the two paths this module runs at the door**: all four of 20.4's session-end events, and the `flush(player)` that drains the `Accrue` buckets — the very mechanism 20.0's headline claims the `ServerMain` ordering exists to protect. So the batched half of the economy silently loses whatever accrued since the last 60 s tick, on every departure, and the session-end distributions have never existed. **Established by elimination, not by guessing:** `Telemetry.Custom` increments `Stats.custom[eventName]` BEFORE it calls `send()`, and `send()` is a `pcall` that raises `failed` and `lastError` on a rejection. Measured at the stop: no `Session*` key at all, `failed = 0`, `lastError = nil`, `sent` unchanged. So the counter line never ran and the function returned at a guard above it — and of the two guards above it, `eventName` is a string literal at all four call sites and `player` is non-nil (`SessionEnd` tested it on the same value one line earlier). `player.Parent` is the only remaining branch. The fix is to let the leave paths through — either a separate entry point that skips the parent test, or make the guard `if not player then return end` and rely on the engine's own argument validation, which 20.0 proved is real. **Whichever is chosen, the `ServerMain` ordering in 20.0 must keep holding**, because `flush` still has to read the balance out of `PlayerDataService.Cache` before PDS's own handler clears it | Stop Play with a hook connected last; `SessionMinutes` / `SessionEndEvolveBarPct` / `SessionEndClaimable` / `SessionEndTimers` all present and `sent` risen. **FIXED AND THE FIX IS MEASURED — BUT THE DIAGNOSIS ABOVE WAS WRONG, AND THAT IS THE IMPORTANT PART OF THIS ROW.** The measurement that was owed was taken 2026-08-20 and it CONTRADICTS the elimination: inside a `PlayerRemoving` handler, **`player.Parent` is `Players`, not nil** (`typeof = Instance`, `Players:FindFirstChild(name) ~= nil`). So the parent guard was never what refused these calls, and `reachable()` behaves identically to the old guard for every call on this path. **What the patch changed and what it did not:** `Telemetry.Custom` / `Economy` / `Funnel` now ask `reachable(player)`, which is a no-op here; `flush` and `SessionEnd` were split into two independent `pcall`s so neither can cancel the other; `leaving[userId]` marks the departure. Since the first is a no-op and nothing throws (`failed = 0`, `lastError` nil), **the `pcall` split is the only change that can account for the behaviour — which is exactly the alternative this row said needed excluding, and it was not excluded.** Measured before: zero session-end events and a leave flush that sent nothing, `sent` frozen at 29 through both `PlayerRemoving` and `BindToClose`. Measured after: three session-end events, leave flush sending (`autoCollect` 1 → 2 → 3), `sent` 6 → 11. **The console also shows WHY the first diagnosis went wrong:** a hook connected LAST printed its dump BEFORE the three session-end events appeared, so Telemetry's own handler was still mid-flight — an `AnalyticsService` call yields, and a later-connected handler runs inside that gap. Reading "no `Session*` keys" at that moment is a measurement artifact; the thing that made the old defect real is that `BindToClose` did not see them either. **STILL `[ ]`: the outcome is fixed and verified, the MECHANISM is not.** The measurement that would close it is one line inside `Telemetry.Custom` logging `tostring(player.Parent)` at the moment `SessionEnd` calls it — the parent may well go nil later in the removal sequence than any handler can observe from outside. Do not write a cause into this row until that line has been run **CLOSED 2026-08-20 BY THE MEASUREMENT THIS ROW DEMANDED, AND THE CAUSE IT NAMES IS NOT THE ONE IT WAS OPENED WITH.** The probe went INSIDE `Telemetry.Custom` and `Telemetry.Economy` -- not into an outside hook picking its own moment -- and printed the parent on the exact calls `SessionEnd` and `flush` make. Four samples, in order, spanning the whole removal sequence: `Economy:Source/DNA/autoCollect`, `Custom:SessionMinutes`, `Custom:SessionEndClaimable`, `Custom:SessionEndTimers` -- every one of them `parent=Players typeof=Instance oldGuard=PASS`. **The parent does not go nil later in the sequence either**, which was this row's last open hypothesis: all four land in the same `os.clock` millisecond (t=145.122), so there is no later for it to go nil in. The old guard would have passed every one of these calls, `reachable()` is a no-op on this path, and the original diagnosis is dead at the exact point it made its claim. **What is left is the `pcall` split, now the only candidate rather than the unexcluded one.** `flush(player)` was a bare call directly above `Telemetry.SessionEnd`, so one throw inside it stops the drain part-way AND propagates out of the handler before `SessionEnd` is ever reached -- a single cause for both halves of the before-measurement (nothing sent at the door, no `Session*` event in existence), where the parent theory needed a guard to refuse calls it demonstrably passes. It also accounts for `failed = 0`: `send()`'s pcall is the only thing that raises that counter, and the throw was above `send`. **WHICH statement in the old `flush` threw is unrecoverable, and is written here as unrecoverable rather than guessed at**: `Telemetry.lua` enters git history in `ccbca8c` already patched, so the pre-patch body exists on no disk and in no Studio. The split is what would have named it, had it been there at the time. The module's own comment block has been corrected -- it stated the refuted cause as fact, in the file the next agent reads first -- and the probe plus its two call sites are deleted: 23,241 bytes, `luascope` OK, 12 registers, pushed over the bridge and hash-verified `OK`, `loadstring` clean |
| 20.7 | `[x]` | <!-- opened 2026-08-20, pre-existing, found while driving 20.2's group-chest call site --> **`RewardService:202` throws on every group-chest claim: `GameConfig.FormatNumber` does not exist.** `attempt to call a nil value`, stack `HandleClaimGroupChest` ← line 405, captured live. Grepped across the whole of `src/`: **there is no `GameConfig.FormatNumber` anywhere, and exactly one caller — this line.** So the claim has always half-completed: DNA, Diamonds and the potion are all credited on the lines above, then it throws inside the `Notify` message, so `PlayerDataService.UpdateLeaderstats` and `PushToClient` never run and the function never returns `"ok"`. The player is paid but sees no confirmation and no updated HUD until something else pushes. **Nothing to do with Phase 20** — the telemetry lines sit above it and all fired (`ChestOpened = 1` and both `groupChest` economy rows were recorded). The formatter exists as `UITheme.FormatNumber`, which is a client UI module and must not be required on the server; `BossService` solves the same problem with a local `formatNumber` at its line 129, and copying that is the minimal fix | Claim the group chest and read the notification. **FIXED AND VERIFIED LIVE 2026-08-20.** A local `formatNumber` was added to `RewardService`, copied from the `BossService` original for the reason written there, and the call site now uses it. Driven through the real `Remotes.ClaimGroupChest` with the daily gate reopened — which costs nothing in the end state, because the handler rewrites `ClaimedGroupChest = os.time()` to the same day it already held. The client received exactly one notification: **`reward :: 🎁 Claimed Daily Group Chest! +438.58B DNA, 💎 +25, 🧪 +1 Potion`** — the formatted number is the very expression that used to throw — and the console is clean where the `RewardService:202` traceback stood. `luastruct` / `luascope` clean, `luaremotes` 60/60, `luanames` OK on both changed files; pushed and hash-verified byte-identical to `src/` (RewardService 16231, Telemetry 22148) with `loadstring` reporting COMPILES on both |
**THAT BLOCKER IS CLOSED (2026-08-20, later the same day).** `AnalyticsService` accepts every
argument this game builds when the player is a real `Player`: **29 calls, `failed = 0`**, plus the
three API calls made raw outside the wrapper, all `true`. The engine confirms each one in the
console by itself. What the session cost, and what the next one should not re-derive:

- **`start_stop_play` works exactly ONCE per Studio launch.** The first call returns `Game Started`;
  every later call — start or stop — returns `Start play hasn't finished yet` forever, while every
  other MCP tool keeps answering instantly. Only a **Studio restart** clears it, and the restart
  re-registers the plugin under a **new `studio_id`**. Hash `src/` against Studio before asking for
  one (118 scripts, all identical, is what made it free). **Batch everything that needs Play into
  the single window you get** — including the stop, which is the only chance to observe
  `PlayerRemoving` / `BindToClose`.
- **`Telemetry.Stats` cannot be read with a plain `require`.** A probe VM gets a fresh module with
  zeroed counters; `PlayerDataService.Cache` reads empty on a server that loaded the save perfectly.
  Inject a `Script` at runtime instead so it runs in the REAL VM and mirrors `Stats` into a
  `StringValue`. Anything that PERTURBS the save must be restored by that injected script under a
  deadline, not by a follow-up call.

**What the Play session then FOUND was two live defects, 20.6 and 20.7, neither of which any
amount of reading had caught -- and they were the whole reason 20.0-20.4 sat at `[~]`. Both are
closed as of 2026-08-20, so those five rows are closed with them.** The last thing 20.4 was owed
arrived with 20.6's probe: three of its four session-end events fire at the door
(`SessionMinutes`, `SessionEndClaimable`, `SessionEndTimers`). The fourth is **correctly** absent
-- `SessionEndEvolveBarPct` is skipped when `GetEvolveStep(data).isMax` is true, and the save
driving the test is maxed, read straight out of `EvolutionLab_v1` / `Player_5746881443`. That same
read shows `data.Funnel = pre`, which is why no funnel step fires for this account and must not be
read as 20.1 being broken: a save that predates the instrumentation is never logged, on purpose.
...
| ID | | Task |
|---|---|---|
| 21.1 | `[~]` | **Trading is fully built and invisible.** All seven remotes are live and `TradePanel` is 1,011 lines, but the only door is knowing to click another player. Give it an HUD tile and a prompt over nearby players | **CODED AND PUSHED 2026-08-20, NOT YET VERIFIED LIVE -- and one half of the row was deliberately not built.** Built: section 6 of `TradePanel`, a green **Trade** tag over the head of any other player inside `GameConfig.TradeProximityStuds`, which is the SERVER's own range constant (`TradeService` reads the same one), so a tag can never appear over somebody the request would then be refused for. Tapping it opens the same card the 16.2 click opens -- one more way in, not a second implementation -- and that card already carries the picker behind its quieter button, so seeing one human makes the whole feature reachable. A single toast fires the first time anybody is in range in a session and never again. Three choices worth keeping: **not `AlwaysOnTop`**, the same rule the raycast follows, because a tag shining through a wall advertises a trade with somebody you cannot reach; **`ExtentsOffsetWorldSpace`, not `StudsOffset`**, because the body runs 1x to 9x across the twenty stages and a constant offset is a hat at one end of the game and a kite at the other; and the tags **mute while the modal is open**. **THE HUD TILE WAS NOT BUILT, ON PURPOSE, AND IT IS THE ONE THING TO OVERRULE IF YOU DISAGREE.** That tile existed and **16.2 deleted it on the owner's own direction** -- trading means nothing until somebody is standing next to you, and a permanent button spent a fifth of the left edge asking the player to find a name in a list while they were looking straight at the person. Putting it back undoes a closed row, breaks the left column's 2x2 block, and costs a register in a file that has none. The tag answers the complaint the row actually makes -- nothing tells you the feature is there -- at the only moment it is true. **THE GEOMETRY HALF IS NOW DONE, AND IT WAS WRONG. 2026-08-20, single client, stage 20, four captures.** The row named the tag's own geometry as the risk and it was right. `ExtentsOffset` IS MEASURED IN THE ADORNEE'S HALF-SIZE, NOT ITS SIZE -- measured, not read: three billboards side by side on one Head showed `ExtentsOffsetWorldSpace = 2.0` rendering at exactly the height of `StudsOffset = head.Size.Y`, and far below `StudsOffset = head.Size.Y * 2`. So `1.8`, written and reviewed as "1.8 head heights", was **0.9**. That put the tag *under* `CombatClient`'s `HealthPlate` -- the name plate on every head, anchored at `head.Size.Y * 1.35 + 1.6`, which is the line the costume shell and the late-stage crowns end at. The gap is `0.9H - (1.35H + 1.6) = -0.45H - 1.6`: **negative at every one of the twenty stages and worse as the body grows** -- 2.6 studs inside the costume at Cell, 4.7 at Alien, 6.8 at The Absolute (measured: 10.36 against the plate's 17.13). On a tag that is deliberately not `AlwaysOnTop`, so the body occludes it. **The affordance built to stop trading being invisible was being swallowed by the player it points at.** A bigger number could not have fixed it either: the plate is anchored in studs and sized in PIXELS, so the studs it covers grow with camera distance, and `CameraFit` alone moves that from 12.5 to 46 before the other player's 40 studs of range are added -- any constant stud offset slides through it at some distance. **The fix**: `alignPrompt` puts the tag on the plate's OWN anchor and pushes it clear in the plate's own pixel space with `SizeOffset`, so the gap is identical at every stage and every distance. Both numbers are READ OFF THE PLATE (`plate.StudsOffset`, `plate.Size.Y.Offset * (plate.SizeOffset.Y + 0.5)`) rather than copied, so this never becomes a second place to edit when `CombatClient` moves them; the literal fallback only covers the frames before its `WaitForChild` returns. Re-aligned every poll pass, because an evolve grows the head under a tag whose Adornee never changed. Captured before/after on one head: shipped tag at the top of the silhouette, fixed tag sitting one clean 6 px gap above the name. **A CAPTURE BLIND SPOT WORTH THE ROW: no `AlwaysOnTop` BillboardGui renders in this Studio at all** -- the name plate was missing from the first three captures and the comparison read as "there is nothing up there to collide with". Forcing `plate.AlwaysOnTop = false` is what made the shot honest. **THE TWO-CLIENT RUN HAPPENED 2026-08-20 AND IT FOUND A SECOND DEFECT, IN A LINE THAT WAS NOT 21.1's.** Two clients, 7 studs apart, driven from both sides. **Verified working:** the tag appears over the other player and drops when they leave range (measured at 7, 290 and 351 studs); both sides get one, not just the asker; the geometry holds on a LIVE body -- `StudsOffset` equals `HealthPlate.StudsOffset` exactly (4.7069) and `SizeOffset.Y` is 1.95, which is the plate's own `52 * (0.5 + 0.5) + 6 + 20` over the tag's 40, and a capture shows the green pill one clean gap above the name; and **the mute works**, exercised through a REAL trade rather than a poked flag -- `TradeRequest` from one client, auto-accepted on the other, modal up within 0.25 s and the tag gone by the next 0.4 s poll. **THE DEFECT: THE TAG NEVER COMES BACK.** `currentTradeId` is set in the `TradeUpdate` handler and was cleared NOWHERE -- not by the cancel button, not by the `cancelled`/`completed` branch, which clear `currentSession` and `myOfferIds` beside it. `muted = tradeModal.Visible or currentTradeId ~= nil`, so **after a player's first trade of the session the trade tag is gone for the rest of it**, on both sides. Measured: modal closed, distance 7.03, both heads present, three further poll passes, no tag on either client. **The shape is worth more than the fix.** `currentTradeId` had been assigned-and-never-read since the panel was written -- a dead write, and dead writes do not have to be correct. 21.1 gave it its first reader and inherited a variable nobody had ever needed to maintain; the review that read the poll line saw a plausible variable name and stopped. See [[optional-arg-nothing-passes]] for the same shape from the other side. Fixed in both places, pushed and hash-verified (49,499 bytes), four lints clean. **Still owed, and it is now ONE run:** the first-fire of the toast (only its never-again half is proven -- the tag was re-created after a range drop and no second toast came, but the first one had already fired before the probe was armed), and the tap itself. **THE TAP CANNOT BE TESTED BY AN AGENT AT ALL:** `user_mouse_input` does not hit-test BillboardGuis. Proven, not assumed -- the cursor was placed on the tag's exact centre (confirmed against a capture, and nothing of the HUD overlaps that point) and not even `MouseEnter` fired, while the identical call on a ScreenGui button opened the Shop panel. `VirtualInputManager`, the one engine-level route left, is refused in this context (`lacking capability RobloxScript`), same as `Lighting.Technology`. **SECOND RUN, AFTER THE FIX, 2026-08-20 -- everything else now verified.** The fix holds: tag drops the moment the modal opens and is back within one 0.4 s poll after the trade ends (measured across a full request/accept/cancel cycle at 4.18 studs, both sides). **The toast fires exactly once**, caught at its FIRST firing rather than inferred -- a temporary probe planted in `StarterPlayerScripts` before Start recorded `t=6.27  Player1 is nearby -- tap the tag above them to trade`, and the session's complete toast list afterwards is that one plus `Trade cancelled` from the trade itself: no repeat, across two tag destroy/recreate cycles. (The probe was needed because a toast lives 4 s and one MCP round trip is 5-15 s; note that **`execute_luau` has its OWN `_G`**, separate from the game's LocalScripts -- the bridge back is a second LocalScript created at runtime that copies `_G` onto its own attributes. Probe removed, tree swept.) **And `showCard`'s clamp was measured at all four extremes** with the card actually placed there: 10 px inside the top, left and right edges, 68 px from the bottom -- the tap can never open a card half off the screen. That check nearly produced a false alarm and is worth the warning: `AbsolutePosition` in this UI is measured from UNDER the topbar, so window = abs + 58, and a card reporting `top = -48` is really 10 px down the screen. Calibrate against a known element (`ShopButton` at abs y 63) before calling anything off-screen. **👤 OWNER, and it is the only thing left: tap the green tag in a two-client run and confirm the 16.2 card opens.** Three seconds. Everything the tap depends on -- the button exists, is `Active`, is `Visible`, has nothing over it, and the point it hands `showCard` lands the card on screen -- is verified |
| 21.2 | `[x]` | <!-- coded + pushed + verified 2026-08-20 --> **The daily ladder stops after day 7.** Loop it at a higher tier, and show the day-7 reward from day 1 as the thing being climbed towards | **CODED, PUSHED AND MEASURED LIVE 2026-08-20 -- one five-line branch is on disk but NOT in Studio, see the end of this row.** **The tier.** `GameConfig.DailyTiers` is four rungs -- Week 1 x1, Week 2 x2, Week 3 x4, Veteran x8 -- and the last one repeats for ever, because a ladder with no top is a promise the economy cannot keep. `GetDailyTier(streak)` and `GetDailyReward(streak, dayIndex?)` are the only two ways to turn a streak into a payout, and the day-index arithmetic `((streak - 1) % 7) + 1` that had been written out by hand in **five** places is now written once: a tier added to four of them would have been a board that promises what the server does not pay. **THE MULTIPLIER IS ON THE DNA ONLY, AND THAT IS THE LOAD-BEARING LINE.** DNA is the one reward here that runs through `ScaleReward`, so it is authored in stage-1 clicks and x8 means the same thing at stage 1 and at stage 20. Diamonds, Shards and potions are FLAT numbers aimed at fixed sinks -- a diamond upgrade costs 5/8/15 and never moves -- which is exactly why 1.1 kept them out of `ScaleReward`; multiplying those by 8 would buy every permanent upgrade in the game off one month of logins. The tier pays them as `bonusDiamonds` instead, **on the hero day only** (+1/+2/+3), so day 7 climbs 2 -> 3 -> 4 -> 5 diamonds and nothing else moves. **The board.** The subtitle names the week and its multiplier (suppressed at x1, where there is nothing to say); every cell repaints at the player's own tier on each DataUpdate, since the labels were authored once at build time off the week-one row and that is correct exactly until the first Monday of a second week; and the hero column gets a purple goal pill in the one gap nothing owns -- the plinth ends at 244 and the amount line starts at 280, while the same gap on a small tile is TWO pixels, which is why only the hero has one. It says three different things: `N DAYS TO GO` while the week runs, `CLAIM IT TODAY!` on the day it lands, and -- once day 7 is collected -- the NEXT tier, which is the one moment the loop is worth advertising. The banner does the same on the night the week closes, because `Come back tomorrow for Day 1!` to somebody holding a seven-day streak is the exact deflation this row was opened against. **A defect fixed on the way past:** the day cards drew both Shards and Diamonds with the SAME gem glyph, so the one card the week climbs towards advertised two diamond payouts of different sizes -- the identical confusion the daily toast was fixed for, left standing on the board itself. Shards are the gold star the HUD pill and that toast already use. | **Server, driven through the real `RewardService.HandleClaim` on the live save, eight claims:** streaks 1/7/8/14/15/21/22/28 pay tiers 1/1/2/2/3/3/4/4 and the DNA ratio between consecutive tiers is **exactly 2.000000** (87.72G -> 175.43G -> 350.87G -> 701.73G at the owner's stage), day-7 diamonds 2/3/4/5, shards flat at 3 in every week. The probe snapshotted and restored `DNA`, `Diamonds`, `EvolutionShards`, `RewardStreak`, `LastRewardClaim` and the whole `Potions` table under `PlayerRemoving` + `BindToClose`, and a second bridge read the save back afterwards: `streak=2`, the real `LastRewardClaim`, untouched. **Client, six board states photographed off the running HUD** by firing doctored COPIES of the save at it (the save itself never changed): week 2 mid-week shows `Week 2 -- DNA x2` with 400/900/2.00K...46.00K and `4 DAYS TO GO`; week 3 `x4`; Veteran `x8` with 184.00K on day 7 and 5 diamonds; day 7 claimable shows `CLAIM IT TODAY!` and the `CLAIM!` chip; day 7 just claimed shows `Week 2: DNA x2` and the banner `Week complete -- Week 2 starts tomorrow, DNA x2!`; ticks correct in all six. Capture taken of the real board. Four lints clean, `luaremotes` 60/60, MainUI 145 registers. **THE LAST BRANCH WAS THE ROW'S OWN COMPLAINT, AND IT IS CLOSED.** Case D (Veteran, day 7 claimed) first measured the banner as `Come back tomorrow for Day 1!`: the top tier has no next rung to promise, so it fell out of the branch and the deflation this row was opened against was landing on the only player who has climbed the entire ladder. It says `Week complete -- Veteran keeps paying DNA x8!` now, **read off the running HUD after a Studio restart and a re-push** (270,937 -> 271,502 bytes, `loadstring` COMPILES), together with `TOP TIER x8` on the goal pill and `Week 2: DNA x2` on the week-one case beside it. **And the text was measured rather than trusted**: `TextFits` and `TextBounds` both describe the truncation, so every new string was re-measured with `GetTextBoundsAsync` at its label's own shrink floor -- subtitle 203x12 in a 592x22 box at the worst case this feature can produce (a 365-day streak on the longest tier name), goal pill 119x14 in 168x22, banner 280x14 in 622x28: whole with margin, all of them. The goal pill sits at y 409..437 against a plinth ending at 405 and an amount line starting at 441 -- clear of both, on the live body. **Two traps this run bought.** A capture taken with the panel at `UIScale 1.25` pushes it past the top of the viewport, so a header that is fine reads as clipped -- check the geometry at the shipped scale before believing a photograph of a scaled panel. And **a `StringValue` marker replicates SLOWER than a `RemoteEvent`**, so two samples came back labelled with the wrong case: the client already held the next payload while the marker still said the previous tag. The data self-labels (the subtitle prints the streak) which is what caught it; a probe whose only label is a replicated property is not labelled at all. Save re-read afterwards: `RewardStreak = 2` and the original `LastRewardClaim`, untouched. |
| 21.3 | `[x]` | <!-- coded + pushed + measured live 2026-08-20; the rollover push measured in a LIVE TICK 2026-08-21 -->**A persistent daily playtime ladder** beside the session one. The session gifts correctly reward long sittings and stay; a player with three 20-minute sessions is served by nothing | **CODED, PUSHED AND MEASURED LIVE 2026-08-20, AND CLOSED 2026-08-21 when the one branch that had never run -- the tick's rollover push -- was measured in a live tick. See the last paragraph.** **THE GAP, STATED PRECISELY, BECAUSE IT IS NOT THE ONE THE ROW'S WORDING SUGGESTS.** The session claim set is already day-stamped and persisted (1.x closed the rejoin faucet), so the fragmented player is not paid three times -- they are paid **once, ever, for the whole day**. Three twenty-minute visits take the 10 and the 20 on the first visit and then find every rung they can reach already spent; visits two and three earn nothing at all. So the fix is not a cheaper or repeatable session ladder, it is a **second clock**. **THE ACCUMULATOR.** `data.DailyPlaytime = { day, seconds, claims }`, stamped with the same UTC day the session claim set uses, so it resets once a day and never on a rejoin. **IT IS BANKED BY A 30-SECOND TICK, NOT BY A LEAVE HANDLER, AND THAT IS THE LOAD-BEARING LINE.** The obvious shape -- add the session's length on `PlayerRemoving` -- cannot work here and it fails **silently**: `PlayerDataService.Init` runs first, so its `PlayerRemoving` is connected first, and that handler **saves and then drops the cache**; anything this service wrote afterwards would go into a table nothing reads again. A tick also survives the two ways a session actually ends most often -- a crash and a timeout -- neither of which runs a leave handler at all. The cost is bounded: at most 30 s of the final minute, against a ladder whose finest rung is thirty minutes. **`accrue()` IS THE ONLY WAY TO READ THE TOTAL**, and every caller goes through it rather than reading `seconds` -- the banked number is up to a tick stale, and the player in front of a button that has just turned green is exactly the case that trips on it. It clamps the delta at zero (an NTP correction can step `os.time()` backwards, and a negative delta would take time **off** a total that only grows), and on a rollover it resets the row **and** `lastTick` together, dropping the sub-tick sliver that belonged to yesterday rather than splitting it. `lastTick` is in memory and never in the save: it is a mark on the server's clock, and a stale one restored from a save read a week later would bank a week. **THE RUNGS: 30 / 60 / 120 / 180 / 240 minutes.** They start where the session ladder's third rung is because this ladder has to be reachable by the player the row was opened for -- three twenty-minute visits -- and **unreachable by accident in one short sitting**, or it is just the session ladder paying twice for the same ten minutes. The DNA is big (3K / 8K / 25K / 60K / 150K) and the diamonds are not (1 and 2), the same line 21.2 drew: `dna` runs through `ScaleReward` so it is authored in stage-1 clicks, while diamonds are FLAT and aimed at fixed sinks. Worth stating why the diamonds are a garnish and not a faucet: **kills already pay ~260 diamonds an hour** (measured, GameConfig.Diamonds), so four hours of play earns about a thousand from combat against the three this ladder adds. **ONE `HandleClaim` FOR BOTH LADDERS.** Everything after the gate -- grants, telemetry, the two pushes, the toast -- is identical; only the clock and the claim set differ, and both are data. The remote takes an optional second argument and **every value that is not the string `"daily"` is the session ladder**, so a payload of one number still claims exactly what it always did; no new remote, `luaremotes` still 60/60. **THE BOARD: BOTH LADDERS IN ONE PANEL, UNDER A BANNER EACH.** Two panels would have made the one comparison that matters -- "the 30-minute rung is spent, but the OTHER 30-minute rung is still open to me" -- a comparison across a click. The banner is a `CustomLayout` card rather than a bare label dropped into the scroll, which keeps it inside the `UIListLayout` ordering and inside the padding that stops a 4 px stroke being sliced off by the clip, and **it carries the clock, live** (`TODAY IN TOTAL - 2h 15m played`) -- the only place the accumulated total is ever shown, and what turns a label into a number the player can watch move. The daily clock crosses the wire as a **virtual start time** (`dailyStart = now - secondsToday`) rather than a duration, so `os.time() - start` is the right expression for both ladders and the client needs no second countdown and no record of when the payload landed. Two smaller calls: past an hour the countdown drops its seconds field (`179m 04s` is a stopwatch nobody watches), and the card titles now read `30m / 1h / 2h` -- which changes the SESSION titles too, from `60 minutes` to `1h`, deliberately, so the title, the countdown and the button agree and the two ladders can be read against each other. | **SERVER, driven through the real `PlaytimeGiftService.HandleClaim` on the live save from a Script injected into the running VM** (a fresh `require` gets a zeroed `PlayerDataService` cache and every call returns at its first line). **The accumulator banks**: 0 -> 28 s across one real 30 s tick -- and 28 rather than 35 is itself the proof that a raw read is a tick stale, which is why `accrue` exists. **Every rung, in order, against its own threshold: all five refuse one minute under and pay at it**, with diamonds +0/+0/+1/+0/+2 exactly as authored and `xp_l` 0 -> 1 on the top rung. The DNA is `ScaleReward` of the authored number to a **ratio of 1.000000**. A second claim on a paid rung is refused; a bad index and an unknown ladder name are refusals, not crashes. **The two claim sets are independent in BOTH directions** -- five daily claims left the session set at 0, and (waiting out the real ten-minute session clock, because the first attempt was inconclusive when the claim was simply refused) a session claim that **succeeds** moved the session set 0 -> 1 and left the daily set at 0. **Backdating the row resets the seconds AND hands every rung back**, then correctly refuses the claim. **CLIENT, photographed off the running HUD**: both banners and all ten rungs, countdowns correct on every one (session 5/15/25/40/55 against 10/20/30/45/60; daily 25m/55m/1h 55m/2h 55m/3h 55m against 30/60/120/180/240), the dark banner reading clearly as furniture between two amber cards with no overlap between its title and its right-aligned hint. **All three toast shapes caught at their firing** by a recorder armed before the claims: `Playtime Gift (10 min)` for the session ladder unchanged, `Daily Playtime Gift (30 min)` and -- the case the formatter exists for -- `Daily Playtime Gift (1h)`, not `60 min`. **The save was snapshotted and restored under `PlayerRemoving` plus `BindToClose`, and the restore was AUDITED rather than trusted** -- which is how the run's one real scare was settled: DNA read back 1.518e15 above the restored figure, and a 20-second sample of the idle income measured 6.474e12/s, predicting 1.554e15 over the elapsed 240 s. The gap is the game's own income, not probe residue. Four lints clean, MainUI unchanged at 145 registers, every new string measured with `GetTextBoundsAsync` on the string (banner 329x22 in 376x52 at the widest a day can print, hint 162x15 in 200x52, button `3h 59m` 72x25 in 160x45) -- a measurement that had already caught the banner authored full-width **underneath** the right-anchored hint, truncating 200 px early. **THE RUN FOUND A DEFECT THAT WAS NOT IN THE ROW, AND IT IS THE ONE WORTH KEEPING.** `pushStatus` fires on join and on claim **and nowhere else**, so a day rollover was invisible to the client: the server hands the ladder back at 00:00 UTC while the panel still shows yesterday's rungs greyed to `DONE`. Nothing on that panel is then claimable, so no claim is ever fired, so no push ever happens -- **the ladder stays shut for the rest of the session for the one player who was there to see it turn over**, and the same is true of the session ladder, which has had this since it was written. Measured rather than argued, with a client sampler recording the panel once a second: server rolled over at t=45 (real `accrue`, row reset to 0 seconds and 0 claims), the panel held `30m played, DONE=2` for **twelve more seconds**, and a push at t=57.7 turned it into `0m played, DONE=0, WAIT=5`. The fix is three lines -- `accrue` returns whether it rolled, and the tick that noticed pushes; only the tick acts on it, since `pushStatus` calls `accrue` itself and must not push from inside it -- and a save with no row yet is deliberately not a rollover, so a fresh join does not fire a spurious push. **MEASURED IN A LIVE TICK 2026-08-21, which is what closed this row.** A client sampler was armed first, logging both channels -- every payload landing on `PlaytimeStatus`, and the board as drawn once a second -- and the server probe was injected as a Script so its `require` reached the real cache. It fired **exactly one push, ever** (the setup: `dailyClaimed=2`, board `30m played, DONE=2`), then back-dated the row and touched nothing again; its watch loop only reads. **14.2 s later a second push arrived that nothing in the probe sent** -- `dailyClaimed=0`, and a `dailyStart` exactly 1820 s past the seeded one, which is the 1800 s seeded plus the 20 s elapsed, i.e. a row whose `seconds` is zero rather than a re-send of the same payload. The board went to `0m played, DONE=0, WAIT=5` on the next refresh. **The restore was read back from the real VM by a separate audit script**: `DailyPlaytime = {day, seconds=1500, claims={}}` -- 1500 s is the pre-probe 24m plus the two 30 s ticks between the restore and the audit, so it reconstructs itself -- the two doctored rungs gone, the session set never moved. **One thing cost the first attempt and is worth keeping:** the probe's HTTP report never landed, because **five processes were listening on 127.0.0.1:8731** -- Windows lets them stack -- and the POST was answered by a leftover GET-only `http.server` with a 501. A GET returning 200 is not evidence the bridge is healthy. The channel that worked instead is a `StringValue` in the real datamodel, which `execute_luau` can read even though it can never see the module cache. **One probe fault worth recording, because it is a shape and not a typo:** the first probe aliased its snapshot instead of copying it, and its `setMinutes` helper reused the SAME `claims` table across every doctored row -- so "restore" handed back the original table whose `claims` field had been mutated, leaving rungs 1 and 5 marked claimed on a real save. Caught by auditing the restore, repaired in place, and the rewritten probe deep-copies. **A restore that is not read back is not a restore.** |
...
| ID | | Task | Check |
|---|---|---|---|
| 26.1 | `[x]` | <!-- coded + pushed + verified 2026-08-20 --> **The event ladder.** `GameConfig.EventQuests[eventKey]` -- an ordered board of four rungs per event, reusing the four counters `SeasonPassService.Track` already feeds (`creatures`, `bosses`, `eggs`, `fuse`), each rung paying Diamonds or DNA and **only the last one paying the character**. Progress lives in `data.EventQuests[eventKey] = { window, progress, claimed }` bucketed by the **window's start timestamp**, so each occurrence is its own board -- which is exactly right for ColosseumClash, whose champion rotates weekly and should have to be re-earned. `EventService.GrantRewards` stops granting the skin on login | a save online for the whole window with an untouched ladder owns no skin; the same save with four rungs claimed owns it | **BOTH HALVES OF THE CHECK MEASURED, on the real code pushed into Studio and required fresh.** A save present for **48 hourly `SyncCharacters` calls right across a live window** with the ladder untouched ends with `EventCharacters=0` and `Characters=0`; the same shape of save that claims the four rungs ends owning **`event_clash_onyx`**, copied into `Characters`. **The login grant is GONE, not disabled** -- `EventService.GrantRewards` is now `nil` and the executable diff against HEAD removes exactly the grant loop and its two call sites and nothing else (21 code lines out, `git diff` filtered to non-comments). What replaced it is `EventService.SyncCharacters`, renamed deliberately: **a function that grants nothing, called from the join hook under the name GrantRewards, is the defect shape [[optional-arg-nothing-passes]] is about** -- the name is what stops anyone reading it. It still runs on join and on every poll because a rebirth can wipe `Characters` at any moment and there is nothing live to re-grant an ended event from. **THE SAVE DESIGN IS ONE COMPARISON**: `GetEventBoard` rebuilds any board whose stamp is not the live occurrence's `startTs`, the same trick `GetQuestPeriod` plays with `periodId` -- so the reset is correct after a restart, correct for a player who was offline when the window turned over, and cannot be missed. Measured: window A's completed 4/4 board carried **0 progress rows and 0 claimed rows** into window B, the champion moved `event_clash_onyx` -> `event_clash_ember`, the save kept last week's skin, and `cc_4` on the new board **refused**. **PROGRESS RUNS IN PARALLEL; THE CLAIM IS WHAT IS ORDERED**, and that is a decision rather than a shortcut: the counters are shared with the season board, so gating the COUNTER would mean a boss advancing the weekly quest and silently not the event, which a player has no way to diagnose. Order is enforced at the button, where it is visible. Two rungs on one counter are therefore cumulative -- **15 then 35 bosses put `cc_4` at 50, not 65**. **THE WIRING COST ZERO NEW CALL SITES.** `Track` gained one loop over `GameConfig.AdvanceEventQuests`, which is pure over the save and the clock; the five gameplay call sites were not touched. Proven with a control: **midweek** `Track(creatures,30)` moved the season daily to 30 and created **no event board at all**; **in-window** the same calls moved daily creatures/eggs/bosses to 30/7/2 and `cc_1`/`cc_3`/`cc_2`/`cc_4` to 30/7/2/2 off the same calls. **EVERY GATE DRIVEN THROUGH THE REAL HANDLER** (real `EventService` + real `SeasonPassService`, stubbed only at the data/announce layer): claim at 0/400 refused and nothing paid; `cc_2` before `cc_1` **with all four targets met** refused; `cc_1` paid **+4 diamonds** and a second press paid nothing; `cc_2` paid 12,000 DNA `ScaleReward`'d to **148,862,389** at stage 10; `cc_3` +8; `cc_4` **+15 and the skin, once** -- a second press changed neither. A rung finished inside the window and claimed **two days later refused**: the window is the door, deliberately and with no grace period, because honouring a late claim means keeping a closed occurrence addressable, which is the stale-multiplier shape the header of `GameConfig.Events` exists to prevent. PrismFest run end to end on its own fixed window (2026-09-04 12:00 .. 09-07 12:00): four rungs claimed in order paid 4/DNA/8/15 and **`event_prism`**; on a fresh save with all four targets met, claiming `pf_4` first **refused and granted no skin**; an hour after the window closed every claim refused. Unknown event, unknown rung and a ladderless event (`Weekend2x`, which has no ladder on purpose -- it hands over no skin) all return `false` without throwing. **THE TARGETS ARE QUOTED AGAINST THE ONLY CALIBRATED BOARD IN THE GAME** -- `QuestPool`'s weekly row (500 creatures / 20 bosses / 60 eggs over seven days) -- at roughly 2-3x it inside a window a third as long, with 11.11's measured 85 kills/min and the 40-72 s boss respawn written into the comment so the next person tuning it is arguing with numbers. Each ladder is themed on its own event's effect: Colosseum ends on bosses (`bossMult`), Prism on eggs (`luckAdd`). **THREE PLACES WERE STILL TELLING THE OLD RULE AND ALL THREE ARE FIXED**: both event blurbs said the skin was *"for everyone who shows up"* (they are drawn on the Forest sign and the HUD boost card), and the **Journal's own hint promised *"Turn up and it is yours for good"*** on two of its three branches -- the game's Journal describing a rule the server no longer runs. They now name the ladder and **read the rung count** (`#GetEventQuests`) rather than hard-coding "four", so a fifth rung does not have to be remembered in a HUD file. **Owed to 26.3**: the card still does not say what the ladder asks or how long is left. **Lints**: `luascope` OK on all five files, `luaregs` clean (Events 6 registers, EventService 29, JournalGrid 13), `codediff` shows SeasonPassService as +14/-2 executable lines (two locals turned into exports plus the one Track loop) and JournalGrid as 4-for-4. All seven touched or dependent scripts `loadstring` clean in Studio including `MainUI` and `GameConfig`. **`luaremotes` reports `ClaimEventQuest` as a door no client fires -- that is 26.2**, and it is the one thing this row leaves open |
| 26.2 | `[x]` | <!-- coded + pushed + verified 2026-08-20 --> **The ladder needs a surface**, and the Season panel's Quests tab is already the quest board. The live event's ladder draws above the daily and weekly ones, in the event's own colour, with the skin drawn as the final prize and the window's countdown on the header. It disappears when the window closes — the market's urgency comes from the board going away, not from a label saying "limited" | **BOTH HALVES MEASURED IN A RUNNING PLAY CLIENT, on the real code pushed over the bridge.** Off a weekend (the real 2026-08-20, a Thursday) the tab draws **9 rows and hides 10** -- daily and weekly only, with the event rows present-but-invisible rather than absent, which is the whole build-once-and-hide decision: a panel built at join would otherwise have to grow rows at 00:00 Saturday for somebody already standing in it, and that is a code path which only ever runs when nobody is watching for it. Inside a window the same tab draws **two ladders at once**: `ColosseumClash` at LayoutOrder 1-5 and `PrismFest` at 21-25, above daily at 41-45 and weekly at 46-49. **TWO LIVE LADDERS IS THE AUTHORED SCHEDULE, NOT A HYPOTHETICAL** -- PrismFest runs 4-7 September and the Colosseum opens 00:00 Saturday the 5th, so the launch weekend has both boards on. They are ordered by the same `priority` rule `GetActiveEvents` sorts by, because the Forest sign, the HUD boost card and `GetEventHeadline` all draw `active[1]` and a fourth surface headlining a different event is exactly what 12.13 already paid for once; `Weekend2x` sits between them in the active list, has no ladder on purpose, and correctly takes no slot. **ALL FOUR BUTTON STATES DRIVEN AND PHOTOGRAPHED** on one board: `✅ Done`, `CLAIM!`, and the two locked forms -- `🔒 400 left` for a rung still running and **`🔒 Step 2 first` for a rung whose counter is FULL but whose turn has not come**. That last state is the surface 26.1's parallel-progress decision needed: 120/120 under a locked button reads as an order, where a pressable button the server then refuses reads as broken (the 18.6 fault, and the reason the daily rows stopped saying "Claim" at 0/3). **THE PRIZE IS RESOLVED FROM THE WINDOW, NOT FROM THE CONFIG**, and it is measured: the 2026-09-05 occurrence drew **❄️ Frost Sentinel** on `cc_4` with a chip painted rgb(118,208,255) -- the character's own colour, not the event's -- while `pf_4` drew **🌈 Prism Herald** at rgb(158,120,255). A name written at build time would be right for one weekend in four. **COUNTDOWNS EXACT**: at a probe clock of Sat 12:00 the Colosseum header read `ends in 36h 00m` (the window closes Mon 00:00) and Prism `ends in 48h 00m` (Mon 12:00); the redraw is on the same one-second tick the daily reset countdown uses, so the section leaves at the instant the clock says so rather than on the next DataUpdate. **NEITHER THE SAVE NOR THE DATASTORE WAS WRITTEN.** The clock was moved the way the game itself moves it -- `ReplicatedStorage.LiveEvents`, the payload `PotionTimers` adopts -- rather than by patching a config table, which [[evolution-lab-studio-mcp-gotchas]] measured cannot reach running code from a probe; progress arrived as a **synthetic `DataUpdate` built from the client's own captured payload**, fired back through a temporary RemoteEvent sink. **THE BADGE SUMS BOTH BOARDS**: two pressable rungs on two different events put `2` on the Season tile, which is the only thing that tells a player the ladder exists without opening the panel. **ONE REAL DEFECT FOUND AND FIXED BEFORE SHIPPING, AND IT WAS IN THE FIRST FIX.** The canvas had to stop being `questOrder * 78 + 30` -- a build-time count, wrong the moment rows come and go -- and the obvious replacement, `questLayout.AbsoluteContentSize.Y`, **is measured in screen pixels that have already been through this panel's `UIScale`, while `CanvasSize` is written in offset units that the same scale then multiplies a second time**. Measured live at three scales: at 1.0 the two agree and the bug is invisible; **at 0.5 the rule would have written a canvas of 728 against 698 absolute pixels of content -- half the board unreachable -- and at 0.35, 518 against 488**. Every panel in the game carries a fit scale clamped to `[0.35, 1]` (`registerPanel` in MainUI), so that is every phone, and it would have read as "the quests tab does not scroll on mobile". Replaced with `AutomaticCanvasSize = Y`, which the engine computes in the frame's own space and which `ScrollAffordance` already recognises and pads -- 14 px tail confirmed on the shipped build. **Lints**: `luascope` OK, `luaregs` 16 registers / 184 of headroom, and `luaremotes` now reports **59 remotes all with a speaker and a listener** -- `ClaimEventQuest`, the door 26.1 left open, is fired. `loadstring` clean in Edit, pushed byte-identical, and a full 116-file rolling-hash sweep shows Studio and `src/` in sync with nothing else stale. Console clean and MainUI built all 41 children. **Owed to 26.3**: the Journal card, which is the other place the ladder has to be legible |
| 26.3 | `[x]` | <!-- coded + pushed + verified 2026-08-20 --> **A locked event slot in the Journal says `?` and nothing else.** It must name its event, say what the ladder asks and how long is left, or the whole ladder is invisible to the person who has not opened the Season panel today | a locked event portrait names the event and its window | **BOTH SURFACES MEASURED IN A RUNNING PLAY CLIENT, on the real code pushed over the bridge and `loadstring` clean.** The card is the ladder now: for an unowned event skin the detail panel drops its stat box and its hint and draws the event's title with the window's clock in the event's own colour, one row per rung, and a foot line -- **off the weekend** (the real Thursday 2026-08-20) `event_clash_ember` read `Colosseum Clash - runs in 1d 23h`, the four rungs `1. Defeat 400 creatures` .. `4. Defeat 50 bosses`, and `Week 1 of 4 - its turn in 8d 23h`; **inside its own weekend** (probe clock moved to noon on the Saturday the rotation hands over ember) the same card read `ends in 1d 11h`, a tick / `15/15` / `42/120` / `15/50` with the two finished rungs' counts in green, `Step 4 hands over this skin`, and a door button painted rgb(226,84,76) -- **the event's own colour, not the kit's green**. **THE RUNG COUNT IS READ, NEVER WRITTEN**: the rows are built once for the longest ladder in the config and the spare ones hide, so a fifth rung costs no edit in this file. **THE PROGRESS COLUMN IS BLANK UNLESS THE WINDOW IS OPEN ON THIS SKIN, AND THAT IS THE ROTATION'S DOING RATHER THAN CAUTION** -- ColosseumClash runs for one of its four champions at a time and the counters advance for all of them, so `120 / 400` on the card of a skin this weekend cannot hand over would be the loudest thing on it and a promise the claim then refuses. Those cards get the requirement, the clock and their own turn instead. **THE DISC ITSELF ANSWERS THE ONE QUESTION A ROW OF FIVE `?`s CANNOT**: a locked event cell lends the damage caption's slot to its window, measured on the real client through `MainUI.refreshCharacterPanel` -- ember `LIVE NOW` in green on its weekend, `in 8d 23h` off it, Prism `in 15d 11h` -- and **the caption is put BACK when the skin is owned**, proven on `event_clash_verdant`, which this save actually holds and which correctly drew `= best` beside four counting-down siblings. Nothing else in the panel moved: the caption stays hidden for every ordinary locked disc (one added disjunct, `refs.entry.event ~= nil`, and `codediff` shows MainUI as exactly **+1/-1 executable lines**), and selecting an ordinary skin, then the event skin, then the ordinary one again restores stat box, hint and Equip button every time -- driven and read back in one VM. **THE BUTTON IS A DOOR, AND ONLY WHILE THE WINDOW IS OPEN**: `hud.showSeasonPanel` gained an optional tab and the Journal passes `"quests"`, because a button saying *open the event board* that lands on the XP track is the 18.6 fault; measured on the real `SeasonPass` module -- `TrackPage` false, `QuestPage` true. Off the window there is no button at all rather than one that opens a board with no section for this event. **ONE REAL DEFECT, FOUND BY THE CAPTURE AND BY NOTHING ELSE.** The foot line is authored in the card's subtitle grey (126,134,156), whose luminance **0.526 sits above `UITheme.IsDarkInk`'s 0.45 cut** -- so `themeLabel` handed it the full 4 px `Color.Outline` halo and it shipped as dark ink inside a dark halo on a pale card, the 15.1 blob, while the four rung labels above it (authored at 0.306) had dropped their strokes at build time. Every property read correct. `inkOnLight` -- this card's own helper, cut at 0.62, owning `Thickness` **and** `Transparency` -- now runs on the foot and on every count label that changes colour. A second capture fix went with it: the rungs are 18 px, not 19, because `styleButton`'s lip draws outside its frame and the 4 px the foot had cleared the button by rendered as nothing. **The countdowns tick on their own second** (the same guarded one-second loop the Season panel's own timers use) rather than on the next `DataUpdate`, so the card stops offering a board at the instant the window shuts. **HOW IT WAS DRIVEN, since no agent can click in Play**: the real `JournalGrid` module was built a second time against a synthetic `hud` table and a **real `DataUpdate` payload captured off the wire**, which is the only route to a card whose only selector is a mouse. **Lints**: `luascope` OK on all three files, `luaregs` 13 top-level / 40 inside the closure of 200, `luaremotes` 59 remotes all with a speaker and a listener, and the full 116-file rolling-hash sweep shows Studio and `src/` identical with nothing else stale |
| 26.4 | `[x]` | <!-- coded + pushed + verified 2026-08-20 --> **The VIP wardrobe has no storefront.** A locked VIP portrait gets a Robux door and the row header says what the pass costs. This is the row that turns an existing, correct, invisible piece of monetisation into revenue | clicking a locked VIP portrait opens the Robux prompt for the VIP pass | **THE ROW AS WRITTEN NAMED THE WRONG REMOTE, AND IT WOULD HAVE SHIPPED AS A DEAD BUTTON.** VIP is a GAME PASS (`GameConfig.GamePasses`, key `VIP`, passId 1941409673, 499 R$), not a developer product -- and `PromptRobuxPurchase`, the one the Season card uses, resolves its argument through `GetRobuxProduct`, which searches `RobuxProducts` and must never contain a pass: a product is consumed through `ProcessReceipt`, a pass is owned forever and is checked with `UserOwnsGamePassAsync`. Firing it with `"VIP"` throws nothing and prints nothing -- `RobuxShopService` finds no product and returns -- so the press would have done **literally nothing**, which is the quietest way a monetisation row can fail. The door fires `PromptGamePassPurchase`, the remote `ShopPanel`'s own pass tiles have fired since 19.12, and it sends the **key** rather than the id so a tampered client can only ever name a pass that exists. **BOTH HALVES MEASURED IN A RUNNING PLAY CLIENT** on the real code pushed over the bridge: with the pass off the save the header reads `☀️ VIP Exclusive • R$ 499 for all 9` and both VIP portraits carry a Gold rgb(255,198,45) button reading `👑 Get VIP — R$ 499`, active and hover-lit; with the pass on it the header reads `• all yours` and the same button is `✓ Wearing it` / `Wear this one`. **THE FIXTURE HAD TO BE MANUFACTURED, AND THAT IS THE ENVIRONMENT WALL THIS ROW FOUND**: `PassService.Refresh` grants **every pass** unconditionally under `RunService:IsStudio()` (it logs `STUDIO TEST MODE` on every join), so the unowned half of this row **does not exist in Studio at all** -- it was reached by doctoring one field of a real `DataUpdate` captured off the wire and handing it to the real `JournalGrid` built a second time against a probe-owned `hud` table, the route the probe-routes note describes. **The same wall means `MarketplaceService:PromptGamePassPurchase` itself cannot be reached from Studio for any pass** -- the server refuses an owner before it prompts. What WAS driven end to end is the wire: the client fired `PromptGamePassPurchase:FireServer("VIP")` and the server answered `You already own VIP!`, which is `PassService` resolving that exact key to that exact pass row; a bogus key and a numeric pass id both produced no notify and no error, so both guards hold. That leaves one owner-side task: **one real press on the published place**, the same thing `ProcessReceipt` already owes. **THE TEST IS ON THE PASS, NOT ON THE SKIN, and the two disagree for the length of one join.** `SyncVipCharacter` writes all nine into `Characters` off `data.Passes`, and `PassService` fills `Passes` on its own `PlayerAdded` connection with retries -- so `not owned` alone would offer a paying VIP a till they have already been through, and the server would answer its best customer with `You already own VIP!`. The door tests `GameConfig.OwnsPass` and hides on `passId <= 0`, and `paintVipHeader` drops the price on the same condition, so the row goes quiet together rather than advertising a price with no button under it. **CONTROLS, ALL ON ONE SAVE**: an ordinary locked skin, the worn skin, an owned skin and an off-window event skin are byte-for-byte what 26.3 shipped (no button, `✓ Wearing it`, `Wear this one`, no button), and the sequence VIP -> ordinary -> VIP -> event restores every time -- the two door flags cannot stick and cannot both be true, because an entry is either `vip` or `event`. The count is read (`#VipCharacters`) and so is the price (`GetGamePass("VIP").price`), so a tenth bundle or a re-price needs no edit in this file. **ONE PRE-EXISTING DEFECT FIXED ON THE WAY**: the `equipped` branch switched `AutoButtonColor` off and nothing ever switched it back, so looking at the worn skin and then at any other one left a live Equip button with no press feedback for the rest of the session. **Lints**: `luascope` OK, `luaregs` 13 top-level and 45 of 200 inside the closure, `luaremotes` 59 remotes all with a speaker and a listener, `codediff` **+33/-1 executable lines with nothing else moved**. `loadstring` clean, pushed byte-identical (rolling hash 371345271 on both sides), and a 116-file sweep shows Studio and `src/` in sync with nothing stale. Console clean and MainUI built all 42 children. **Owed**: nothing here; 26.5 is the Exhibit |
| 26.5 | `[x]` | <!-- coded + pushed + verified 2026-08-20 --> **The Exhibit** — the owner's *"da stoje likovi izlozeni negde na pocetku igre"*. A pedestal row near spawn in `HubPlaza`: the nine VIP skins (real catalog bundles, so they can be worn by a display rig) and the event skins, each on a plinth with a sign saying how it is earned, and **unowned ones silhouetted** rather than hidden. Locked-but-visible is the whole mechanism — it is what every game in the research does with the thing you cannot buy today | walk spawn and the two sets are standing there, silhouetted, each naming its price or its ladder | **FOURTEEN STANDS, BOTH RANKS, BUILT AND MEASURED IN A LIVE PLAY SERVER AND PHOTOGRAPHED FROM FOUR ANGLES.** Two ranks flank the walk down from the spawn — the nine VIP bundles east at x = 56, the five event skins west at x = -56, both starting at z = 330 on a 24-stud rhythm, each figure scaled to a single **9-stud display height** and standing on a plinth whose top is `DECK_TOP + 6.5` (measured: every figure's feet at y = 6.54 on the first live build). **THE STATUE IS THE SKIN, NOT A MODEL OF IT**: `SkinMesh.TemplateFor` is the same lookup the costume path uses, and the yaw is SkinMesh's own arithmetic including `FaceFlip` — the five event templates carry that attribute and the nine VIP ones do not, so re-deriving the rule here would have turned five statues round. **THE SILHOUETTE IS THE CLIENT'S AND IT WAS MEASURED THERE, ON A RUNNING PLAY CLIENT**: every locked event stand read **6 of 6 mesh parts at rgb(46,42,66) with 0 textured**, and the one this save actually owns — `event_clash_verdant` — read **0 dark, 6 textured, `✓ YOURS` in rgb(58,150,84)**, which is the control that says the paint is ownership and not a blanket. **CLEARING `MeshPart.TextureID` IS THE HALF A TINT DOES NOT COVER**: these are catalog bundles, so `Color` alone leaves a fully textured Korblox-grade body standing there in a dark wash, which reads as a lighting bug rather than as a locked item. It is a local write on a replicated part and the original is kept, so a pass bought mid-session repaints the row without a rejoin. **THE CLOCKS ARE REAL AND TICK ON THEIR OWN SECOND**: off the weekend the four champions read `⏳ its turn in 1d 12h` (onyx, this Saturday), `8d 12h` (ember), `15d 12h` (frost) and the festival `⏳ runs in 15d 0h` against its authored 4 September window. **LOCKED IS THE DEFAULT, DELIBERATELY**: the figures replicate in their true colours, so a client that never reaches a DataUpdate would show fourteen unlocked skins — the one wrong answer that costs a sale — and every stand is silhouetted the moment it is found, before any payload arrives. **THE WARDROBE IS TESTED ON THE PASS, NOT ON THE SKIN**, for 26.4's reason: `PassService` fills `data.Passes` on its own retrying connection and `SyncVipCharacter` only writes the nine keys once it has, so testing the skin alone would silhouette a paying VIP's own row for the first seconds of their session. **The Studio wall from 26.4 is still there** — every pass is granted under `RunService:IsStudio()`, so all nine VIP stands read `✓ YOURS` in Studio and the locked half was photographed instead, by making the client's exact three writes on four stands in the Edit world. **TWO REAL DEFECTS, BOTH FOUND BY A CAPTURE AND BY NOTHING ELSE.** The plaque was authored 4.9 studs out from the plinth centre — inside the base's own 13-stud footprint — so it photographed as *"Cybe  un / VIP Pass · R$  ll 9"* with the column standing in front of the middle third of its own label, while every property read correct and `TextFits` was true. It clears the base's boundary along the facing now (6.5 / cos 20° = 6.9) with both far corners of a 9-stud slab outside it, and its bottom edge sits ON the deck so it reads as a planted label. The second was the plinth itself: a 13-stud base under a 3-stud column photographed as a low table with somebody standing on it, and it is a tapered 11.6 / 8.8 / 10.2 now. The event line lost two words for the same reason — *"finish its 4-step ladder"* wrapped inside a `TextScaled` label and a label that wraps shrinks, so that row came out half the height of the VIP row beside it. **A RANK KEEPS ITS RHYTHM, AND THE LIVE WORLD IS WHAT PROVED IT HAD TO**: Forest's group chest at (48, 335) blocks the first VIP plinth, `standAt`'s shared nudge list tries z first, and on the first live build it slid that plinth 14 studs down the rank — two 13-stud bases 10 studs apart, overlapping into one lump of stone. `standInRank` steps OUTWARD in x instead and falls back to the shared gate, which still owns the corridor rule and the skipped counter. Driven with a blocker manufactured at those exact coordinates: the stand lands at **(68, 330) with its z intact**, and the rank behind it is untouched at 306 / 282 / 258 / 234 / 210 / 186 / 162 / 138. **NOTHING ELSE IN THE PLAZA MOVED.** The one piece here whose position is not authored is the photo spot, whose last-resort scan walks x = ±76 with a 40-stud clearance and reaches the east rank, so the exhibit records where it actually stood and the scan reads it — deliberately narrower than a general reservation list, because `PHOTO_FOOT` is a clearance rather than a solid and the four authored photo spots already overlap a lamp's clearance without either being wrong. **THE PRICE AND THE COUNT ARE READ, NEVER WRITTEN**: `GetGamePass("VIP").price`, `#VipCharacters` and `#GetEventQuests(event)`, so a tenth bundle, a re-price or a fifth rung needs no edit in this file, and `passId <= 0` drops the price the same way 26.4's door does. **ENVIRONMENT**: Studio's play toggle wedged after the second start of the session — *Start play hasn't finished yet* for fifteen minutes with the Edit datamodel answering normally — so the **final** geometry was verified in Edit against the real module required fresh through a throwaway clone (the Edit cache hands back the first pull), photographed, and the Edit-built plaza deleted afterwards so the saved place carries no plaza a server would then skip rebuilding. The client half and the first full build were measured in Play before that. **Lints**: `luascope`, `luanames` and `luastruct` clean on both files, `luaregs` 75 registers with 125 of headroom on `HubPlaza` and 18 on `Exhibit`, pushed byte-identical and `loadstring` clean |
...
| `[ ]` | **One real Robux purchase** on the published place — the only thing that exercises `ProcessReceipt` and Roblox's billing. Studio's `IsStudio()` pass grant tests effects, never purchases. **It cannot be faked from a probe and now it is proven why: `ProcessReceipt` is a Roblox *callback member*, so it can be assigned but never read** — the live handler cannot be invoked, and `require` hands back a fresh service whose cache is empty. Cheapest route is any 49 R$ tile. **26.4 adds the pass half of the same wall**: `PassService.Refresh` grants every pass under `IsStudio()`, so the server refuses the prompt as an owner before `PromptGamePassPurchase` is ever called -- the Journal's new VIP door is proven up to and including the server resolving the key, and no further. One press on the published place closes both | 1.7, 2.11, 3.8, 11.12, and now 26.4 |
...
- **2026-08-20 (nineteenth row of the session)** — **21.3: there is a second playtime clock now, it survives leaving, and measuring it found a defect the row never mentioned.** The gap was sharper than the row's wording: the session claim set is already day-stamped, so the player with three twenty-minute visits is not paid three times, they are paid **once for the whole day** — visits two and three find every reachable rung spent. `data.DailyPlaytime` is the second clock, five rungs at 30 / 60 / 120 / 180 / 240 minutes, **banked by a 30-second tick and deliberately not by a leave handler**: `PlayerDataService` connects its `PlayerRemoving` first and that handler saves and then drops the cache, so a total flushed on the way out is written to a table nothing reads again — and a tick also survives the crash and the timeout, which is how sessions usually end. Every read goes through `accrue()` rather than the stored number; the live run made that concrete, banking 0 → **28 s** across one 30 s tick, which is exactly the staleness a claim would otherwise trip on. **Driven through the real `HandleClaim` on the live save from a Script injected into the running VM**: all five rungs refuse a minute under their threshold and pay at it, diamonds land +0/+0/+1/+0/+2 as authored, the DNA is `ScaleReward` of the authored number to a ratio of **1.000000**, a repeat claim is refused, and the two claim sets are independent **in both directions** — the second of which needed the real ten-minute session clock waited out, because the first attempt proved nothing when the claim was simply refused. Photographed off the running HUD with both banners and ten correct countdowns, and all three toast shapes caught at their firing, including the one the formatter exists for: `Daily Playtime Gift (1h)`, not `60 min`. **The run's real find was not in the row.** `pushStatus` fires on join and on claim and nowhere else, so a day rollover is invisible to the client — the server hands the ladder back at midnight while the panel still shows yesterday's rungs greyed to `DONE`, nothing on it is claimable, so no claim is fired and no push ever happens, and **the ladder stays shut for the rest of the session for the one player who was there to see it turn over**. The session ladder has had this since it was written. Measured with a once-a-second client sampler: the panel held `30m played, DONE=2` for twelve seconds after the server had reset the row, then a push turned it into `0m played, DONE=0, WAIT=5`. Fixed in three lines, and **on 2026-08-21 those three lines were measured running in a live tick**: a probe fired exactly one push to seed an end-of-day board, back-dated the row and then touched nothing, and 14.2 s later a push nothing had sent turned `30m played, DONE=2` into `0m played, DONE=0, WAIT=5` -- named as the rollover rather than a re-send by a `dailyStart` exactly 1820 s past the seeded one, a row whose seconds are zero. The restore was read back from the real VM and reconstructs itself: 1500 s is the pre-probe 24m plus the two ticks that ran before the audit. **And one probe fault worth more than the row it appeared in:** the first probe aliased its snapshot instead of copying it and reused one `claims` table across every doctored row, so its "restore" handed back a table it had mutated and left two rungs marked claimed on a real save. It was caught only because the restore was **read back** rather than trusted — the same audit that then explained a 1.5e15 DNA gap as the game's own idle income, measured at 6.474e12/s. A restore that is not read back is not a restore.
- **2026-08-20 (eighteenth row of the session)** — **21.2 is closed: the top-tier banner branch was pushed after the restart and read off the running HUD.** It says `Week complete — Veteran keeps paying DNA x8!` where it used to say `Come back tomorrow for Day 1!`, which was the row's own complaint landing on the only player who had climbed the whole ladder. Captured beside it: the Veteran board with all seven days collected in their own muted hues, 184.00K DNA and 5 diamonds on day 7, and `TOP TIER x8` on the goal pill. **Every new string was measured with `GetTextBoundsAsync` at its label's shrink floor**, because `TextFits` and `TextBounds` describe the truncation rather than the string — the worst subtitle this feature can produce (a 365-day streak on the longest tier name) is 203x12 in a 592x22 box, and the goal pill clears the plinth above it and the amount line below it on a live body. **Two traps worth keeping.** A capture taken at `UIScale 1.25` pushes the panel past the top of the viewport, so a header that is fine photographs as clipped; check the geometry at the shipped scale before believing the picture. And **a `StringValue` marker replicates more slowly than a `RemoteEvent`** — two samples came back labelled with the wrong case because the client already held the next payload while the marker still read the previous tag. The data self-labelled (the subtitle prints the streak), which is the only reason it was caught; a probe whose label is a replicated property is not labelled at all. Probes swept, client's view restored, save re-read: `RewardStreak = 2` and the original `LastRewardClaim`.
- **2026-08-20 (seventeenth row of the session)** — **21.2 built and measured: the daily board loops at a higher tier now, and the week you are on is written on it.** Four rungs (x1 / x2 / x4 / Veteran x8, the last repeating), and the multiplier lands on the DNA ONLY — that is the whole design decision. DNA runs through `ScaleReward`, so it is authored in stage-1 clicks and x8 means the same thing at every stage; Diamonds, Shards and potions are flat numbers aimed at fixed sinks (an upgrade costs 5/8/15 and never moves), which is why 1.1 kept them out of `ScaleReward` in the first place, and multiplying them by 8 would buy every permanent upgrade in the game off a month of logins. The tier pays diamonds instead as a small fixed add on the hero day only: 2 → 3 → 4 → 5 across the four weeks. **Measured through the real grant, not read**: eight claims driven through `RewardService.HandleClaim` on the live save give tiers 1/1/2/2/3/3/4/4 for streaks 1..28 and a DNA ratio of exactly 2.000000 between consecutive tiers; the probe snapshotted and restored every field it could touch under `PlayerRemoving` plus `BindToClose`, and a second bridge read the save back to prove it (`streak=2`, real timestamp, untouched). **Six board states photographed off the running HUD** by firing doctored COPIES of the save at the client — week 2 / week 3 / Veteran all repaint their cells, the subtitle names the week and suppresses the multiplier at x1, and the hero column's new goal pill reads `N DAYS TO GO`, `CLAIM IT TODAY!` or the next tier depending on where the week stands. **Two things worth keeping.** A client write to a ReplicatedStorage `StringValue` does not replicate up, so the first command channel was silently one-way and every case read back as the real save — the server has to drive the sequence and the client follow a marker it publishes. And the board had been drawing Shards and Diamonds with the SAME gem glyph, so day 7 advertised two diamond payouts of different sizes: the exact confusion the daily toast was fixed for, still standing on the board the toast describes. **21.2 is `[~]` for one branch and one push.** Case D (Veteran, day 7 claimed) measured the banner as `Come back tomorrow for Day 1!` — the top tier has no next rung, fell out of the branch, and so the row's own complaint was landing on the only player who has climbed the entire ladder. Fixed on disk and lint-clean, but `start_stop_play` is wedged for this launch (once per Studio launch, as the tenth row records), so Edit is unreachable: **Studio holds MainUI at 270,937 bytes and disk holds 271,502, and the difference is exactly that branch.** Restart Studio, push MainUI, re-read case D.
- **2026-08-20 (sixteenth row of the session)** — **The fix was re-run on two fresh clients and 21.1 is down to one owner click.** Tag drops when the modal opens and is back within one 0.4 s poll after the trade ends — a full request/accept/cancel cycle at 4.18 studs, both sides, where before the fix it never returned. **The toast was caught at its first firing**, not inferred: a probe planted in `StarterPlayerScripts` before Start recorded `t=6.27  Player1 is nearby — tap the tag above them to trade`, and the whole session's toast list afterwards is that one plus `Trade cancelled` — no repeat across two tag destroy/recreate cycles. Two things that make such a probe work at all and are easy to lose: a toast lives 4 s while one MCP round trip is 5–15 s, so it must be recording before the run starts; and **`execute_luau` has its own `_G`**, separate from the game's LocalScripts, so the way back is a second LocalScript created at runtime that copies `_G` onto its own attributes. **`showCard`'s clamp was measured rather than read**, with the card actually placed at all four extremes: 10 px inside top, left and right, 68 px off the bottom. That one nearly produced a false alarm worth keeping — **`AbsolutePosition` in this UI is measured from under the topbar**, so window = abs + 58, and a card reporting `top = -48` is really 10 px down the screen. Calibrate against a known element before calling anything off-screen; `ShopButton` sits at abs y 63 and photographs at 163. The only thing an agent can never do here is the tap itself — `user_mouse_input` does not hit-test BillboardGuis and `VirtualInputManager` is refused for lacking `RobloxScript`. **21.1 stays `[~]` on a 👤 OWNER line**: click the green tag in a two-client run, confirm the card opens, and it is `[x]`. Probe removed and the tree swept; nothing this session planted is left in the place.
- **2026-08-20 (fifteenth row of the session)** — **The two-client run finally happened, and the trade tag turned out to switch itself off permanently after one trade.** Everything 21.1 built checked out on live bodies: the tag appears on BOTH sides at 7 studs and drops when the other player leaves range (measured at 290 and 351); the re-aimed geometry holds against a real `HealthPlate` (`StudsOffset` identical at 4.7069, `SizeOffset.Y` 1.95 = the plate's own pixels), and a capture shows the pill one clean gap above the name; and the mute fires through a REAL trade — `TradeRequest` from one client, auto-accepted on the other, modal up in 0.25 s, tag gone on the next 0.4 s poll. **Then it never came back.** `currentTradeId` is written by the `TradeUpdate` handler and was cleared by nothing — not the cancel button, not the `cancelled`/`completed` branch that clears `currentSession` and `myOfferIds` on the lines beside it. Since `muted` reads it, **a player's first trade removed the trade tag for the rest of their session**, on both clients, with the modal shut and the other player 7 studs away. **Why it survived review: it was a dead write.** Assigned once, read by nobody, for the whole life of the panel — and a variable nobody reads never has to be right. 21.1 gave it its first reader and inherited the debt; the poll line reads perfectly well if you assume the name means what it says. Fixed in both places, pushed over the bridge and hash-verified (49,499 bytes, `loadstring` clean), `luastruct`/`luanames`/`luascope`/`luaregs` all clean. **Two tooling facts bought along the way.** `user_mouse_input` **cannot click a BillboardGui** — cursor on the tag's exact centre, verified against a capture, with nothing of the HUD over that point, and not even `MouseEnter` fired; the same call on a ScreenGui button opened the Shop panel. And the AlwaysOnTop question from the last row is **settled the other way**: the engine draws them fine, `screen_capture` is what omits them. An A/B in Edit — one creature plate cloned, one flag different — showed only the `false` copy in the capture while Kristina saw **both** in her viewport. Nothing is wrong with the game's name plates or its popups. **21.1 stays `[~]`**: one more two-client run owed for the toast's first fire and for the tap, which is hers to click because no agent can.
- **2026-08-20 (fourteenth row of the session)** — **21.1's tag was hanging inside the player's own body, and the number that put it there reads correct.** The row named the tag's geometry as the risk and the capture it asked for found the tag under the name plate rather than over it. **`ExtentsOffset` is measured in the adornee's HALF-size** — measured on a live client, three billboards on one Head: `ExtentsOffsetWorldSpace = 2.0` renders at exactly the height of `StudsOffset = head.Size.Y`, far below `StudsOffset = head.Size.Y * 2`. So the shipped `1.8`, written and reviewed as "1.8 head heights", was **0.9**, which is `0.45H + 1.6` studs *below* the line `CombatClient`'s `HealthPlate` uses to clear the costume shell and the late-stage crowns — negative at all twenty stages and worse as the body grows, 2.6 studs inside the costume at Cell and 6.8 at The Absolute. On a tag deliberately not `AlwaysOnTop`, so the body occludes it: **the affordance built to stop trading being invisible was being swallowed by the player it points at.** A bigger constant would not have fixed it, because the plate is anchored in studs and sized in pixels, so the studs it covers grow with camera distance and `CameraFit` alone moves that from 12.5 to 46. The fix stacks the tag on the plate in the plate's own space — its anchor, then `SizeOffset` in the plate's pixel height — with both numbers read off the plate instead of copied, and re-applied every poll pass so an evolve cannot leave it behind. **The session's own trap, and it nearly buried the finding: no `AlwaysOnTop` BillboardGui renders in this Studio at all.** The name plate was absent from three captures, which read as "nothing up there to collide with"; forcing `plate.AlwaysOnTop = false` is what made the comparison honest. Pushed and hash-verified; `luascope` and `luaregs` clean. **21.1 stays `[~]`**: the two-client run is still owed, now for the interaction only.
- **2026-08-20 (thirteenth row of the session)** — **21.1 coded: trading has a tag over the person now, and no tile.** A green Trade pill sits over the head of any other player inside the server's own 40-stud trade range and opens the card 16.2 built, which already carries the picker behind it — so the whole feature becomes reachable from seeing one other human, which is the only moment trading means anything. The row also asked for an HUD tile and **it was not built**: 16.2 removed that tile on the owner's own direction, and putting it back would undo a closed row, break the left column's 2x2 block and cost a register MainUI does not have. That is a design call and it is written into the row to be overruled rather than buried. **Not verified live**: a tag only exists over a second player, and `start_stop_play` is wedged for this Studio launch — it answered `Game Stopped` once for 20.6 and has hung on every call since, which is the once-per-launch behaviour already in the notes. Pushed over the bridge, hash-verified `OK`, `loadstring` clean.
- **2026-08-20 (twelfth row of the session)** — **20.6 closed, and Phase 20 with it — but the row closes on a cause it did not open with.** The one measurement it was still owed was taken from INSIDE `Telemetry.Custom` and `Telemetry.Economy`, on the exact calls `SessionEnd` and `flush` make at the door: `parent=Players`, `typeof=Instance`, `oldGuard=PASS`, on all four, across the whole removal sequence and inside one `os.clock` millisecond of each other. **A player is not unparented inside `PlayerRemoving`, not at the first call and not at the last** — so the guard this row was opened against never refused anything, and `reachable()` is a no-op on the path it was written for. What is left standing is the `pcall` split: a bare `flush(player)` above `Telemetry.SessionEnd` let one throw take both reports, which is one cause for both halves of the symptom and also explains why `failed` never moved (`send()`'s pcall is the only thing that raises it, and the throw was above `send`). Which statement threw is gone for good — the file enters git already patched — and the row says so rather than filling the gap in. Two things came with it. **The wrong cause was sitting in the module's own header as fact**, which is worse than in a row, because it is what the next agent reads before touching the file; it is rewritten. And **the missing fourth session-end event is not missing**: `SessionEndEvolveBarPct` is skipped for a maxed save, confirmed against the real DataStore record, the same read that shows this account is `Funnel = pre` and therefore correctly emits no funnel step at all.
- **2026-08-20 (eleventh row of the session)** — **20.7 fixed and verified; 20.4's events now arrive; and the 20.6 diagnosis written one hour earlier was FALSIFIED by the measurement it asked for.** `RewardService` had called a formatter on GameConfig that has never existed, with exactly one caller — so every group-chest claim paid out and then threw before telling the player. A local `formatNumber` copied from `BossService` fixes it, proven by driving the real remote: one notification, `+438.58B DNA`, and the traceback gone from the console. In `Telemetry`, `flush` and `SessionEnd` were split into two independent `pcall`s and the entry guards routed through a new `reachable()`. **The session-end events now fire** — `SessionMinutes`, `SessionEndClaimable`, `SessionEndTimers`, `sent` 6 → 11, `failed = 0` — and the leave flush sends where it previously sent nothing. `SessionEndEvolveBarPct` is correctly absent on an `isMax` save. **But the stated cause was wrong: `player.Parent` inside `PlayerRemoving` measures as `Players`, not nil**, so the guard was never what refused the calls and `reachable()` is a no-op on that path — which leaves the `pcall` split as the only change that can explain the fix, i.e. exactly the alternative the earlier elimination had set aside. The console also shows how the first reading went wrong: a handler connected LAST printed before the session-end events, because an `AnalyticsService` call yields and a later handler runs inside that gap. **20.6 therefore stays `[ ]` with the outcome fixed and the mechanism explicitly open**, and the row now forbids writing a cause into it until one line inside `Telemetry.Custom` has logged the parent at the moment `SessionEnd` calls it. Both files pushed and hash-verified (16231 / 22148), `loadstring` COMPILES, `luastruct` and `luascope` clean, `luaremotes` 60/60.
- **2026-08-20 (tenth row of the session)** — **Phase 20's one blocker is closed, and the Play session it needed found two live defects instead of clearing the phase.** `AnalyticsService` accepts every argument this game builds when the player is a real `Player`: **29 calls, `failed = 0`, `lastError = nil`**, with the engine printing its own confirmation per call, plus three raw API calls made outside the wrapper that all returned `true`. Funnel steps **3, 4, 5, 7 and 8 all fired through the real doors** — `AutoAttack` fired from the Client datamodel so it crossed the network, a Critter at 20.1 studs taken 30 → 0 hp, then `TeleportToZone`, `BuyEgg` and `HandleEvolve`. Economy proven in **both directions for DNA and Diamonds** and as a Sink for Shards; the 60 s batching confirmed working end to end. **Two things had to be perturbed and both were restored under a dead-man switch owned by the injected script, not by a follow-up call:** the veteran `Funnel = { pre = true }` stamp, and — because the owner's save is at `isMax` with the whole 100-step chain collected — one stage-20 skin briefly un-owned so an evolve had a next rung. Verified back to the byte afterwards: `XP = 35799231`, `worn = vip_tenko`, `isMax = true`, `abs_thefinal` owned, `Funnel = { pre = true }`. **NOTHING WAS MARKED `[x]`, AND THAT IS THE RESULT.** 20.4 was **falsified**: not one of its four session-end events has ever been sent, because `Telemetry.Custom` and `Telemetry.Economy` both refuse any call made from `PlayerRemoving` where `player.Parent` is already nil — which also silently loses the batched economy on every departure, the exact mechanism 20.0's `ServerMain` ordering claims to protect. Established by elimination from the counters, not by guessing (new row **20.6**). And driving 20.2's group-chest call site surfaced a pre-existing crash: `RewardService:202` calls `GameConfig.FormatNumber`, which **does not exist anywhere in the codebase** and has exactly one caller — so every group chest pays out and then throws before the player is told (new row **20.7**). Neither was fixed; both are reported. Operationally: **`start_stop_play` works exactly once per Studio launch** and only a restart clears it, and **`Telemetry.Stats` cannot be read with a plain `require`** — the route is a `Script` injected at runtime so it runs in the real VM. Both written to memory.
- **2026-08-20 (ninth row of the session)** — **Phase 20 opened and built: the game emits something now.** It emitted nothing at all before — no `AnalyticsService`, no funnel, no economy events — which for a soft launch on 4 September means a quiet launch rather than a measured one. New `ServerScriptService/Telemetry.lua` is the only file in the game that calls `AnalyticsService`, and 45 economy call sites across 20 files, 9 funnel hooks and 9 custom events now route through it. **Three things worth keeping.** The wrapper **counts every call into `Telemetry.Stats` before forwarding it, and counts a rejected argument apart from an accepted one** — without that there is no check available to anybody at all, because the service records nothing in Studio and the dashboard answers 24 hours later in a published place. **Half the economy had to be batched**: auto-collect pays every player every second, which is 3,600 events per player per hour and 108,000 an hour on a full server for one faucet, so `Accrue` buckets those and flushes once a minute with an exact total and the balance read at flush time — while a purchase stays a single immediate event, because merging the one event that pays for the game into a minute-wide lump is the same mistake from the other side. And **the funnel is stamped on the save rather than inferred**: `Load`'s fresh branch writes `Funnel = {}` and its returning branch writes `{ pre = true }`, deliberately **not** through `defaultData`, whose backfill loop would have handed every veteran an empty funnel and entered a thousand-hour player at step 8 with no steps 1-7 under them. One finding about the game rather than the instrumentation: **steps 5 and 6 are seconds apart** because here the tutorial *is* the first evolve, so the drop-off to read is 4 → 5. `luascope` earned its place again — it caught `player` used in `SeasonPassService.GrantReward`, which never had that parameter; the fix threads it through all three call sites rather than leaving an optional argument nothing passes. All 24 files pushed to Studio and hash-verified, 118 scripts compile. **Every row is `[~]`, not `[x]`, and one click is why**: `start_stop_play` timed out with Studio still in Edit, so the one thing Edit cannot answer — whether `AnalyticsService` accepts these arguments when the player is a real `Player` — is still owed.
- **2026-08-20 (eighth row of the session)** — **26.5 closed: the exclusives are standing in the road.** Fourteen plinths flank the walk down from the spawn — nine VIP bundles east, five event skins west — each carrying the real `SkinMesh` template at a 9-stud display height with a planted plaque naming the pass and its price or the event and its ladder. An unowned figure is **silhouetted, not hidden**, which is the whole mechanism: measured on a running client, a locked stand reads 6 of 6 mesh parts at rgb(46,42,66) with every texture cleared, and the one event skin this save owns reads full colour and `✓ YOURS` beside it. Three things worth keeping. **Clearing `MeshPart.TextureID` is what makes a catalog bundle a shape** — a tint alone leaves a fully textured body in a dark wash, which reads as a lighting bug. **Locked is the default**, because a client that never reaches a DataUpdate would otherwise show fourteen unlocked skins. And **the capture found both defects**: the plaque was authored inside the plinth's own footprint and photographed with the column standing in front of the middle third of its label — `TextFits` true, every property correct — and the plinth under it read as a low table until it was tapered. Studio's play toggle wedged mid-session, so the final geometry was verified against the real module in Edit and the Edit-built plaza deleted afterwards; the client half was measured in Play before it.
- **2026-08-20 (seventh row of the session)** — **26.3 closed: the locked event slot says what it costs.** The Journal's detail card gives up its stat box and its hint for an unowned event skin — both were a restatement of what the player already hits for, because `GetEffectiveRank` on an off-ladder entry *is* the best rung owned — and draws the event, the window's countdown, every rung of the ladder with this save's progress against it, and the step that hands the skin over. The disc in the grid lends its caption slot to the same clock (`LIVE NOW`, or how long until its turn), which is the only thing that tells a player scanning five identical `?`s which one is being handed out this weekend. The green button becomes a door to the Quests tab while the window is open, and is absent when it is not. Two things worth keeping. **The progress column is deliberately blank for a champion whose turn it is not**: the ladder's counters run for the whole event, but the last rung pays *this weekend's* champion, so a count on the other three cards would be a promise the claim refuses. And **the capture found what no probe could**: the foot line's grey sits at luminance 0.526, above `IsDarkInk`'s 0.45 cut and below `inkOnLight`'s 0.62, so it shipped as dark ink inside a 4 px dark halo while the four rungs above it — authored a shade darker — had dropped theirs at build time. One line looking wrong next to four that looked right is exactly the shape of defect that survives a passing probe.
- **2026-08-20 (sixth row of the session)** — **26.2 closed: the ladder has a board.** The Season panel's Quests tab now draws every live event's ladder above the daily and weekly ones, in the event's own colour, with the rung numbered, the prize skin drawn on the rung that pays it, and the window's countdown on the header — and it draws **nothing at all** off a weekend, which is the point: the urgency is the board going away. `ClaimEventQuest`, which 26.1 left as a door no client fired, is now fired; `luaremotes` reports all 59 remotes with a speaker and a listener. Two things were learned rather than guessed. **The prize cannot be written at build time** — ColosseumClash's champion rotates weekly, so both the chip's colour and the words under it are resolved from the window on every redraw, and the 2026-09-05 occurrence correctly drew ❄️ Frost Sentinel where the config alone would have said nothing. And **`AbsoluteContentSize` is the wrong number to size a canvas from**: it is already through the panel's `UIScale`, which then multiplies it again, so the obvious replacement for the old hand-counted canvas would have made the quests tab unscrollable on every phone — 728 of canvas against 698 of content at scale 0.5, measured live before it shipped. `AutomaticCanvasSize` is the number the engine computes in the frame's own space.
- **2026-08-20 (fifth row of the session)** — **26.1 closed: the event skin now has a price.** `GameConfig.EventQuests` is an ordered ladder of four rungs per event, riding the four counters `SeasonPassService.Track` already feeds, with the character paid only by the last rung and only inside the window it was earned in. `EventService.GrantRewards` — the login grant that handed the skin to anyone who happened to be online — is gone, replaced by `SyncCharacters`, which grants nothing and is named so that nobody reads a grant into it. Progress runs in parallel and only the claim is ordered, because the counters are shared with the season board and gating the counter would have made a boss advance one board and silently not the other. *(This entry was rewritten on the same day from the row and the notes: the original was lost when a bad write truncated `ROADMAP.md`, and the file was restored from HEAD — see the note under 26.2.)*
...
- **2026-08-20 (second row of the session)** — **19.6 closed, and it was not documentation debt.**
  ...
  compacted descendant rather than a revert, with four restored numbers corrected against the code
  beside them (the headline is 24 px now, not 26; `stepFor` tests XP, not DNA; the pips are a column,
  not a row; the arrow is 76 px, not 64) and three blocks written new for what 18.23 actually added.
  **But the row said "documentation debt, not a defect" and that was wrong.** The same compaction had
  deleted two guards from `runClimbBeat`, and a code-only diff — comments and blank lines stripped
  from both revisions — showed they were the ONLY executable difference. `if not ramp then return end`
  is live: without it the fourth tutorial beat shows *"Climb to the next terrace"* for seven seconds
  wherever the player is, with no marker and no trail, because `pointAt(nil)` correctly draws nothing.
  **Measured so it is not theoretical: the EventArena is 1,006 studs from the nearest of the 124
  `TerraceRamp`s, past `nearestRamp`'s own 900-stud cut-off.**
  **The general lesson, and it is now a tool.** 391 changed lines is exactly the cover a deleted line
  of code hides under, and "almost all of them comments" is a claim nobody can check by reading.
  `tools/codediff.py` strips both sides and diffs what is left; on this pair the answer was five
  lines. Swept over all twelve files 19.0 rescued into `tools/_studio_pull/`: **eleven are
  code-identical to `src/` and only `FirstJoin` diverged**, which is what makes this an isolated
  incident rather than a systemic one — and that sweep is the reassurance the row could not have
  given without the tool.

- **2026-08-20** — **18.12 and 19.8 closed. The three orphan panels are gone, and deleting them was
  the small half.** `zonesPanel`, `rebirthPanel` and `robuxPanel` came out of `MainUI`: 437 lines,
  21.6 KB, and 16 top-level registers (160 -> 144, so 56 of headroom under Luau's 200 cap where
  there were 40). The live HUD is 10,360 descendants against 12,316 — the 1,956 instances 18.12
  measured, built and refreshed on every payload behind three doors that no longer existed.
  **What the row did not know, and it is the finding of the session: `robuxPanel` still had four
  doors.** 18.11 repointed the Robux *tile* at the new store; it did not touch the `+` on the DNA
  pill, the `+` on the Diamond pill, the egg panel's Auto Hatch button, or the in-world "Robux Shop"
  counter in every Upgrade Emporium. All four still opened the old panel. **The game was shipping
  two different Robux stores and which one a player saw depended on which button they pressed.**
  That also corrects 19.12 in one detail worth keeping straight: the nine passes were unreachable
  from the tile, not from the game — two of those four doors landed on the old panel's pass tab.
  The revenue bug was real; "no player could reach them" was not.
  **The general rule this pays for is 19.12's, one turn further on.** 19.12 wrote *"repointing a
  door is a DELETION of everything that was behind the old one"*. The corollary is that repointing
  *one* door is not repointing the door: what has to be enumerated is every caller of the thing
  being orphaned, and `grep` for the instance name across `src/` is what finds them — three of the
  four were in other files (`HUD/CurrencyPlus`, `HUD/EggShop`) or in a config table
  (`GameConfig.ShopKinds.upgrades`, reached through `shopPanels`).
  All four now go through one `hudRefs.openStore(passKey)`. New `ShopPanel.SetOpen` and
  `ShopPanel.Focus`; the focus is a scroll hint for the one door opened *by* a pass, deferred a
  frame because `ScrollingPanelBuilder.SetOpen` rewinds the canvas **after** its refresh — a
  same-tick scroll is silently overwritten by a line written to fix a different bug.
  **And the one thing that would have gone dark without a word:** `HUD/RebirthBeacon` is a HUD
  element, not a panel element, and the only thing that has ever told it whether to shine was the
  deleted panel's refresh. 115 lines of `refreshRebirthPanel` became a 5-line
  `hudRefs.refreshRebirthReady`. Deleting the block without noticing that would have cost the game
  its only pointer at the one moment worth interrupting for, with no error and no warning.
