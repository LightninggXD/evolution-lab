...
| 21.3 | `[~]` | **A persistent daily playtime ladder** beside the session one. The session gifts correctly reward long sittings and stay; a player with three 20-minute sessions is served by nothing | **CODED, PUSHED AND HASH-VERIFIED 2026-08-20. NOT YET VERIFIED LIVE -- Studio's play toggle wedged before the run, see the end of this row.** **THE GAP, STATED PRECISELY, BECAUSE IT IS NOT THE ONE THE ROW'S WORDING SUGGESTS.** The session claim set is already day-stamped and persisted (1.x closed the rejoin faucet), so the fragmented player is not paid three times -- they are paid **once, ever, for the whole day**. Three twenty-minute visits take the 10 and the 20 on the first visit and then find every rung they can reach already spent; visits two and three earn nothing at all from this system. So the fix is not to make the session ladder cheaper or repeatable, it is a **second clock**. **THE ACCUMULATOR.** `data.DailyPlaytime = { day, seconds, claims }`, stamped with the same UTC day the session claim set uses, so it resets once a day and never on a rejoin. **IT IS BANKED BY A 30-SECOND TICK, NOT BY A LEAVE HANDLER, AND THAT IS THE LOAD-BEARING LINE.** The obvious shape -- add the session's length on `PlayerRemoving` -- cannot work here and it fails **silently**: `PlayerDataService.Init` runs first, so its `PlayerRemoving` is connected first, and that handler **saves and then drops the cache**; anything this service wrote afterwards would be written to a table nothing reads again. A tick also survives the two ways a session actually ends most often -- a crash and a timeout -- neither of which runs a leave handler at all. The cost is bounded: at most 30 s of the final minute, against a ladder whose finest rung is thirty minutes. **`accrue()` IS THE ONLY WAY TO READ THE TOTAL**, and every caller goes through it rather than reading `seconds` -- the banked number is up to a tick stale, and the player standing in front of a button that has just turned green is exactly the case that trips on it. It clamps the delta at zero (an NTP correction can step `os.time()` backwards, and a negative delta would take time **off** a total that only ever grows), and on a day rollover it resets the row **and** `lastTick` together, dropping the sub-tick sliver that belonged to yesterday rather than splitting it. `lastTick` is in memory and never in the save: it is a mark on the server's clock, and a stale one restored from a save read a week later would bank a week. **THE RUNGS: 30 / 60 / 120 / 180 / 240 minutes.** They start where the session ladder's third rung is because this ladder has to be reachable by the player the row was opened for -- three twenty-minute visits -- and **unreachable by accident in one short sitting**, or it is just the session ladder paying twice for the same ten minutes. The DNA is big (3K / 8K / 25K / 60K / 150K) and the diamonds are not (1 and 2), which is the same line 21.2 drew: `dna` runs through `ScaleReward` so it is authored in stage-1 clicks and worth the same fraction of an hour's income at stage 1 and at stage 20, while diamonds are FLAT and aimed at fixed sinks. It is worth stating why the diamonds here are a garnish rather than a faucet: **kills already pay ~260 diamonds an hour** (measured, GameConfig.Diamonds), so four hours of play earns about a thousand from combat against the three this ladder adds. **ONE `HandleClaim` FOR BOTH LADDERS.** Everything after the gate -- the grants, the telemetry, the two pushes, the toast -- is identical; only the clock and the claim set differ, and both are data. A second copy would be a second place for a payout to drift out of step with the board advertising it. The remote takes an optional second argument and **every value that is not the string `"daily"` is the session ladder**, so a payload of one number still claims exactly what it always did; no new remote, `luaremotes` still 60/60. **THE BOARD: BOTH LADDERS IN ONE PANEL, UNDER A BANNER EACH.** Two panels would have made the one comparison that matters -- "the 30-minute rung is spent, but the OTHER 30-minute rung is still open to me" -- a comparison across a click. The banner is a `CustomLayout` card rather than a bare label dropped into the scroll, which is what keeps it inside the `UIListLayout` ordering and inside the padding that stops a 4 px stroke being sliced off by the clip. **The banner carries the clock, live** (`TODAY IN TOTAL - 2h 15m played`): it is the only place the accumulated total is ever shown, and it is what turns a label into a number the player can watch move. The daily clock crosses the wire as a **virtual start time** (`dailyStart = now - secondsToday`) rather than a duration, precisely so `os.time() - start` is the right expression for both ladders and the client needs no second countdown and no record of when the payload landed; it is also self-correcting, since the push repeats after every claim. Two smaller calls: past an hour the countdown drops its seconds field (`179m 04s` is a stopwatch nobody watches), and the card titles now read `30m / 1h / 2h` -- which changes the SESSION titles too, from `60 minutes` to `1h`, deliberately, because the title, the countdown and the button now agree and the two ladders can be read against each other. **Measured in Edit, which is as far as Edit goes:** all five files `loadstring` COMPILE, the config loads with 5 rungs and every `potionId` resolves against `GetPotion`, four lints clean (`luascope`, `luanames`, `luaremotes` 60/60, `luaregs`) and **MainUI is unchanged at 145 registers**. Every new string was measured with `GetTextBoundsAsync` on the string rather than trusting `TextBounds`, at the worst case each label can produce: banner 329x22 in a 376x52 box (`TODAY IN TOTAL - 23h 59m played`, the widest a day can print), hint 162x15 in 200x52, button `3h 59m` 72x25 in 160x45. That measurement caught a real defect first: the banner label was authored `1, -24` -- full width -- **underneath** the right-anchored hint, so it would have truncated 200 px past where it was readable; it is sized to stop before it, with 4 px of slack. **STILL OWED, AND IT IS ONE PLAY SESSION.** A probe is written and staged at `tools/_probe213/probe.lua` -- it must be injected as a **Script** so it runs in the real VM, because a fresh `require` gets a zeroed `PlayerDataService` cache and every `HandleClaim` would return at its first line. It snapshots and restores `DNA`, `Diamonds`, `Potions`, `DailyPlaytime` and `PlaytimeClaims` under `PlayerRemoving` plus `BindToClose`, and checks nine things: that the accumulator banks ~30 s across one real tick; that each of the five rungs refuses at one minute under its threshold and pays at it; that the DNA paid is exactly `ScaleReward` of the authored number; that a second claim is refused; that the top rung pays its diamonds and its potion; that the two claim sets are independent; and that backdating the row resets both the seconds and the claims. The client half is a capture of the panel with both banners and ten rungs. **`start_stop_play` wedged before any of it ran** -- it timed out with Studio still in Edit and has answered `Start play hasn't finished yet` to every call since, start and stop alike, which is the once-per-launch behaviour already in the notes. **Restart Studio and this is one run.** |
...
- **2026-08-20 (nineteenth row of the session)** — **21.3 built: there is a second playtime clock now, and it survives leaving.** The gap was sharper than the row's wording: the session claim set is already day-stamped, so the player with three twenty-minute visits is not paid three times, they are paid **once for the whole day** — visits two and three find every reachable rung spent. `data.DailyPlaytime` is the second clock: seconds played today, added across sessions, with its own five rungs at 30 / 60 / 120 / 180 / 240 minutes. **It is banked by a 30-second tick and deliberately not by a leave handler**, because that shape fails silently here — `PlayerDataService` connects its `PlayerRemoving` first and that handler saves and then drops the cache, so a total flushed on the way out is written to a table nothing reads again; a tick also survives the crash and the timeout, which are how sessions usually end. Every read goes through `accrue()` rather than the stored number, since the bank is up to a tick stale and the player in front of a button that just went green is the case that trips on it. **One `HandleClaim` serves both ladders** — only the clock and the claim set differ, and both are data — and the remote's new second argument is optional, so a payload of one number still claims what it always did. **Both ladders now sit in one panel under a banner each**, because the comparison that matters ("that rung is spent, but the other 30-minute rung is not") must not be a comparison across a click; the daily clock crosses the wire as a virtual start time so `os.time() - start` is the right expression for both. **The text measurement earned its place again**: `GetTextBoundsAsync` on the string found that the banner label had been authored full-width **underneath** the right-anchored hint, so it would have truncated 200 px early. Five files pushed and hash-verified, all `loadstring` COMPILE, four lints clean, MainUI unchanged at 145 registers. **`[~]`, and it is one Play session**: `start_stop_play` timed out with Studio still in Edit and has refused every call since, start and stop alike — the once-per-launch wedge already in the notes. The server probe is written and staged at `tools/_probe213/probe.lua`; it needs a restart, not more code.
