...
| ID | | Task | Verified how |
|---|---|---|---|
| 8.1 | `[~]` | Request / accept with a **proximity requirement**. One live trade per player, a 4s request cooldown, a 40-stud reach checked at request AND commit, with `Remotes.TradeInvite` toast popup on target client. | verified with simulation harnesses and luastruct/luanames |
...
| 8.5 | `[~]` | **Anti-scam.** Server resets both confirmations unconditionally on any offer modification. 3-second hold countdown before commit with countdown banner and visual cues. | verified in TradeService and MainUI |
| 8.6 | `[~]` | **Trading System Wiring & UI** — `TradeService.Init()` wired in `ServerMain.server.lua`, Remotes (`TradeRequest`, `TradeAccept`, `TradeCancel`, `TradeSetOffer`, `TradeConfirm`, `TradeUpdate`, `TradeInvite`) fully hooked. `TradeModal` and `TradeInvitePrompt` built in `MainUI.client.lua` with offer slot grids, pet picker, anti-scam countdown, and 0 top-level registers added. | verified with luastruct & luanames, simulation scripts |
...
| 15.5 | `[~]` | **`TradeService.resolveOfferPets` called `petIndexById` 71 lines above its `local function`** — a nil global, so every `pushSession` threw and no offer could ever be drawn. Moved above its caller | Two clients on the published place: open a trade, offer a pet, and see it appear in both grids (this is 8.6's own check) | **half done, and the missing half is why this stays `[~]`.** What is proven live: a `TradeUpdate` payload carrying `myOffer` and `partnerOffer` now reaches a joined client and **draws both grids** — three pets on each side, in a window that opens by itself (see the 15.6 capture). What is NOT proven: two *real* clients running the real `TradeService.Request → Accept → SetOffer` chain, because Studio's Play Solo is one client and this session had no second one. The payload above was synthesised on the server and fired at the one client, so it exercises the drawing path and the remote, **not** `pushSession`'s own call into `resolveOfferPets`. Also found on the way: the scope fix alone would still have shipped a dead feature — see **15.9** |
...
| 15.10 | `[ ]` | **`ServerStorage/LightConfig/Type.lua:183` reads `value` inside a function whose parameter is `number`.** `lib.isPositiveInt(number)` returns `type(value) == "number" and number > 0 and ...`, so it is testing a nil global and can only ever return false. Found by `luascope.py` on this session's sweep, not by anything in Phase 15. Left open because `LightConfig` is vendored helper code that nothing in the live boot path calls — worth deleting or fixing, but it should not ride in on a UI commit | `luascope.py` reports Type.lua clean, and whatever calls `isPositiveInt` gets a true for a positive integer |
