...
- **2026-08-12** — **11.11 closed: the diamond upgrades were priced for a game where diamonds had no
  ...
  **And measuring it found 11.31 (E4):** 200,000 rolls per tier returned Apex **0.0000**. Both drop
  tables are keyed by tier name, and 11.6 added a fifth tier without adding a row to either — so the
  creature behind the three-rebirth gate pays no diamonds and no shards. That is 11.30 in a different
  table. Logged, not yet fixed.
