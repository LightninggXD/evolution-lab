--[[
	FirstJoin -- the first two minutes of the game, guided (Phase 6.3, restyled 18.22).
...
local STEPS = {
	...
	evolve = {
		...
		subline = "Press the EVOLVE button",
	...
	climb = {
		...
		headline = "Climb up there",
		subline = "Tougher creatures, better drops",
...
-- creature to attack it") and scaled itself down to fit; splitting it into a headline at a fixed 26px
-- and a subline at 18px means the instruction is read first and the explanation second, at sizes that
-- do not move when the sentence changes length.
...
iconChip.Size = UDim2.new(0, 70, 0, 70)
...
iconImage.Size = UDim2.new(0, 46, 0, 46)
...
iconGlyph.Size = UDim2.new(0, 46, 0, 46)
...
--
-- Filled/hollow rather than a "1 / 3" caption: at this size a number is a word to read, and a row of
-- pips is a thing you take in without reading. Each is stroked in ink like every other surface here
-- so it reads as part of the card rather than as three loose circles on it.
local dots = {}
do
	local row = Instance.new("Frame")
	row.Name = "Dots"
	row.AnchorPoint = Vector2.new(1, 0.5)
	row.Position = UDim2.new(1, -20, 0.5, 0)
	row.Size = UDim2.new(0, 68, 0, 20)
	...
	for i = 1, #DOT_ORDER do
		...
		d.BackgroundColor3 = CardKit.WHITE
		d.BackgroundTransparency = 0.6
		d.BorderSizePixel = 0
		d.LayoutOrder = i
		d.ZIndex = 5
		d.Parent = row
		CardKit.Corner(d, 999)
		CardKit.Stroke(d, CardKit.INK, 2)
		dots[i] = d
	end
end
...
local function paint(key)
	...
	subline.Text = s.subline

	local assetId = IconLibrary.Resolve(s.icon)
	if assetId then
		iconImage.Image = assetId
		iconImage.Visible = true
		iconGlyph.Visible = false
	else
		iconGlyph.Text = s.icon
		iconGlyph.Visible = true
		iconImage.Visible = false
	end

	-- A dot is FILLED once its beat has been reached, so the row fills left to right as the player
	-- moves. `done` is not in `DOT_ORDER` and deliberately lights all three: it is the moment beat two
	-- completes, and a row that emptied itself on the celebration would read as progress lost.
	local reached = 0
	if key == "done" then
		reached = #DOT_ORDER
	else
